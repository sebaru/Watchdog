       ...
       if ($this->form_validation->run() == TRUE)
        { $remember = (bool) $this->input->post('remember');
          ...
          if ($this->wtd_auth_model->login($this->input->post('username'), $this->input->post('password'), $remember))
           { if($this->session->user_accesslevel>=6) redirect('admin/dashboard');
                                                else redirect('/');
           }
          else
           { $this->session->set_flashdata('flash_message', 'Username or Password Error');
				         redirect('auth/login', 'refresh');
           }
        }
       else
        { $this->data['message'] = (validation_errors()) ? validation_errors() : $this->session->flashdata('flash_message');
...
/******************************************************************************************************************************/
 function logout()
    ...
    $this->session->set_flashdata('flash_message', 'Session terminée');
    redirect('auth/login', 'refresh');
	 }
