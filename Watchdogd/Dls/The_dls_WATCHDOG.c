...
/******************************************************************************************************************************/
 void Dls_data_WATCHDOG_create_by_array ( JsonArray *array, guint index, JsonNode *element, gpointer user_data )
  { struct DLS_PLUGIN *plugin = user_data;
    ...
    Info_with_prefix( __func__, "dls", tech_id, LOG_INFO,
              "Create bit DLS_WATCHDOG '%s:%s' (%s)", bit->tech_id, bit->acronyme, bit->libelle );
  }
/******************************************************************************************************************************/
/* Dls_data_WATCHDOG_lookup: Recherche un WATCHDOG dans les plugins DLS                                                       */
...
/******************************************************************************************************************************/
 gboolean Dls_data_WATCHDOG_set_from_thread_watchdog ( JsonNode *request )
  { if (!Json_has_member ( request, "thread_tech_id" ))
     { Info_with_prefix( __func__, "dls", Json_get_string ( request, "thread_tech_id" ), LOG_ERR, "SET_WATCHDOG: missing thread_tech_id" );  return(FALSE); }
    if (!Json_has_member ( request, "thread_acronyme" ) )
     { Info_with_prefix( __func__, "dls", Json_get_string ( request, "thread_tech_id" ), LOG_ERR, "SET_WATCHDOG: missing thread_acronyme" ); return(FALSE); }
    if (!Json_has_member ( request, "consigne" ) )
     { Info_with_prefix( __func__, "dls", Json_get_string ( request, "thread_tech_id" ), LOG_ERR, "SET_WATCHDOG: missing consigne" );        return(FALSE); }
    ...
    Info_with_prefix( __func__, "dls", thread_tech_id, LOG_INFO, "SET_WATCHDOG for: '%s:%s'=%d",
              thread_tech_id, thread_acronyme, consigne );
    struct DLS_WATCHDOG *bit = Dls_data_WATCHDOG_lookup ( thread_tech_id, thread_acronyme );
