...
/* Projet Abls-Habitat version 4.7       Gestion d'habitat                                     sam. 09 nov. 2013 13:49:53 CET */
...
/******************************************************************************************************************************/
 static void Stopper_radio ( struct THREAD *module )
  { struct RADIO_VARS *vars = module->vars;
    ...
    if (vars->radio_pid>0)
     { Info( __func__, "radio", thread_tech_id, LOG_DEBUG, "Sending kill to radio pid %d", vars->radio_pid );
       kill(vars->radio_pid, SIGTERM);
       Info( __func__, "radio", thread_tech_id, LOG_INFO, "Waiting for pid %d termination", vars->radio_pid );
...
/******************************************************************************************************************************/
 static gboolean Jouer_radio ( struct THREAD *module, gchar *radio )
  { struct RADIO_VARS *vars = module->vars;
    ...
    Info( __func__, "radio", thread_tech_id, LOG_NOTICE, "Starting playing radio %s", radio );
    vars->radio_pid = fork();
    if (vars->radio_pid<0)
     { Info( __func__, "radio", thread_tech_id, LOG_ERR,
                "CVLC '%s' fork failed pid=%d", radio, vars->radio_pid );
       return(FALSE);
     }
    else if (!vars->radio_pid)
     { execlp( "cvlc", "cvlc", radio, NULL );
       Info( __func__, "radio", thread_tech_id, LOG_ERR, "CVLC '%s' exec failed pid=%d", radio, vars->radio_pid );
       _exit(0);
     }
    else
     { Info( __func__, "radio", thread_tech_id, LOG_DEBUG, "CVLC '%s' is playing pid=%d", radio, vars->radio_pid );
     }
    return(TRUE);
  }
/******************************************************************************************************************************/
        ...
        { pthread_mutex_lock ( &module->synchro );
          ...
          if ( !strcasecmp( tag, "PLAY_RADIO" ) )
           { gchar *radio = Json_get_string ( request, "radio" );
             Info( __func__, "radio", thread_tech_id, LOG_NOTICE, "%s: Diffusing %s", tech_id, radio );
             Jouer_radio ( module, radio );
           }
          else if ( !strcasecmp( tag, "STOP_RADIO" ) )
           { Info( __func__, "radio", thread_tech_id, LOG_NOTICE, "%s: Stopping radio", tech_id );
             Stopper_radio( module );
           }
          else
           { Info( __func__, "radio", thread_tech_id, LOG_DEBUG, "%s: tag '%s' not for this thread", tech_id, tag ); }
          Json_unref(request);
        }
     }
    Stopper_radio( module );
