...
/* Projet Abls-Habitat version 4.7       Gestion d'habitat                                                27.06.2019 09:38:40 */
...
/******************************************************************************************************************************/
 gboolean Send_mail ( gchar *sujet, gchar *dest, gchar *body )
  { gchar fichier[32], commande[128], chaine[256];
    ...
    fd = mkstemp ( fichier );
    if (fd==-1)
     { Info( __func__, "mail", LOG_ERR, "mkstemp failed" );
       return(FALSE);
     }
    ...
    if (write ( fd, chaine, strlen(chaine) ) < 0)
     { Info( __func__, "mail", LOG_ERR, "writing header failed" );
       close(fd);
       return(FALSE);
     }
    ...
    if (write ( fd, body, strlen(body) ) < 0)
     { Info( __func__, "mail", LOG_ERR, "writing body failed" );
       close(fd);
       return(FALSE);
     }
    ...
    if (write ( fd, suffixe, strlen(suffixe) ) < 0)
     { Info( __func__, "mail", LOG_ERR, "writing suffixe failed" );
       close(fd);
       return(FALSE);
     }
    close(fd);
    ...
    system(commande);
    Info( __func__, "mail", LOG_NOTICE, "mail '%s' sent to '%s'", sujet, dest );
    unlink(fichier);
    return(TRUE);
  }
/*----------------------------------------------------------------------------------------------------------------------------*/
