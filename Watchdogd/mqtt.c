...
/******************************************************************************************************************************/
 void MQTT_on_log_CB( struct mosquitto *mosq, void *obj, int level, const char *message )
  { gint info_level;
    ...
    Info( __func__, "mqtt", info_level, "%s", message );
  }
/******************************************************************************************************************************/
...
/******************************************************************************************************************************/
 void MQTT_Subscribe ( struct mosquitto *mqtt_session, gchar *format, ... )
  { va_list ap;
    ...
    if (!topic)
     { Info( __func__, "mqtt", LOG_ERR, "Memory Error for '%s'", format );
       return;
     }
    ...
    if ( mosquitto_subscribe( mqtt_session, NULL, topic, 2 ) != MOSQ_ERR_SUCCESS )
     { Info( __func__, "mqtt", LOG_ERR, "Subscribe to topic '%s' FAILED", topic ); }
    else
     { Info( __func__, "mqtt", LOG_INFO, "Subscribe to topic '%s' OK", topic ); }
