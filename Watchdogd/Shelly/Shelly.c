...
/* Projet Abls-Habitat version 4.7       Gestion d'habitat                                                08.03.2024 23:35:42 */
...
/******************************************************************************************************************************/
 void Run_thread ( struct THREAD *module )
  { Thread_init ( module, sizeof(struct SHELLY_VARS) );
    ...
    if (!string_id)
     { Info_with_prefix( __func__, THREAD_CLASSE, module->current_thread_tech_id, LOG_ERR, "ERROR: No string_id, stopping thread" ); goto end; }
     ...
     { Info_with_prefix( __func__, THREAD_CLASSE, module->current_thread_tech_id, LOG_ERR, "Shelly type '%s' not recognized", string_id ); goto end; }
        ...
        { pthread_mutex_lock ( &module->synchro );
          ...
          if (Json_has_member ( request, "method" ))
           { gchar *method = Json_get_string ( request, "method" );
             Info_with_prefix( __func__, THREAD_CLASSE, module->current_thread_tech_id, LOG_DEBUG, "MQTT: received '%s'", method );
/*---------------------------------------------------- Notify Status ---------------------------------------------------------*/
