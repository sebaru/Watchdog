    ...
    if (icone)
     ...
     { struct CMD_GTK_MESSAGE erreur;
       g_snprintf( erreur.message, sizeof(erreur.message),
                   _("Unable to locate icone %s:\n%s"), rezo_icone->libelle, Db_watchdog->last_err);
...
/**********************************************************************************************************/
 static void Envoyer_icones_tag ( struct CLIENT *client, gint tag, gint sstag, gint sstag_fin )
  { struct CMD_SHOW_ICONE *rezo_icone;
    ...
    hquery = Recuperer_iconeDB( Config.log, Db_watchdog, client->classe_icone );
    if (!hquery)
    ...
    g_snprintf( nbr.comment, sizeof(nbr.comment), _("Loading icones") );
    Envoi_client ( client, TAG_GTK_MESSAGE, SSTAG_SERVEUR_NBR_ENREG, (gchar *)&nbr, sizeof(struct CMD_ENREG) );

    for( ; ; )
     { icone = Recuperer_iconeDB_suite( Config.log, Db_watchdog, hquery );
       if (!icone)
