...
/******************************************************************************************************************************/
 gboolean Enocean_Lire_config ( void )
  { gchar *nom, *valeur;
    ...
    { Info( __func__, "enocean", NULL, LOG_WARNING,
                "Enocean_Lire_config: Database connexion failed. Using Default Parameters" );
       return(FALSE);
     }

    while (Recuperer_configDB_suite( &db, &nom, &valeur ) )       /* Récupération d'une config dans la DB */
    { Info( __func__, "enocean", NULL, LOG_INFO,                     /* Print Config */
        ...
        { Info( __func__, "enocean", NULL, LOG_NOTICE,
...
/******************************************************************************************************************************/
 static int Init_enocean ( void )
  { struct termios oldtio;
    ...
    if (fd<0)
    { Info( __func__, "enocean", NULL, LOG_ERR,
    ...
    while ( liste_modules )
     { module = (struct MODULE_ENOCEAN *)liste_modules->data;
       ...
       if (module->enocean.type == 0x11 && module->enocean.sous_type == 0x00 &&
        ...
        { gint cpt;
              Info( __func__, "enocean", NULL, LOG_DEBUG,
...
/*       Send_Event( g_get_host_name(), NOM_THREAD, EVENT_INPUT, chaine, 0 );*/
      ...
      Info( __func__, "enocean", NULL, LOG_INFO,
    ...
    while( lib->Thread_run == TRUE )                                     /* On tourne tant que necessaire */
     { usleep(1);
       ...
       if (lib->Thread_reload == TRUE)
        { Info( __func__, "enocean", NULL, LOG_NOTICE, "Run_process: recu signal SIGUSR1" );
          lib->Thread_reload = FALSE;
        }

       if (Cfg_enocean.reload == TRUE)
        { Info( __func__, "enocean", NULL, LOG_NOTICE, "Run_process: Reloading in progress" );
       ...
       switch (Cfg_enocean.comm_status)
        { case ENOCEAN_CONNECT:
           { Cfg_enocean.fd = Init_enocean();
             ...
             else { Info( __func__, "enocean", NULL, LOG_INFO,
           ...
           { guchar sync;
             ...
             if (cpt>0)
              { if (sync==0x55) Cfg_enocean.comm_status = ENOCEAN_WAIT_FOR_HEADER;
                else Info( __func__, "enocean", NULL, LOG_DEBUG,
