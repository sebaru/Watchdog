...
/* Sortie: une GList                                                                                      */
/**********************************************************************************************************/
 gboolean Recuperer_paletteDB ( struct LOG *log, struct DB *db, gint id_syn )
  { gchar requete[2048];
...
/**********************************************************************************************************/
 struct CMD_TYPE_PALETTE *Rechercher_paletteDB ( struct LOG *log, struct DB *db, guint id )
  { struct CMD_TYPE_PALETTE *palette;
    ...
    if ( ! db->row )
     { Liberer_resultat_SQL ( log, db );
       Info_new( Config.log, Config.log_msrv, LOG_INFO, "Rechercher_paletteDB: Palette %d not found in DB", id );
       return(NULL);
     }
