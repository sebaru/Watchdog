...
/******************************* Recherche des event text EA a raccrocher aux bits internes ***********************************/
    ...
    if ( ! Recuperer_mnemos_AO_by_text ( &db, THREAD_CLASSE, critere ) )
     { Info_with_prefix( __func__, THREAD_CLASSE, module->current_thread_tech_id, LOG_ERR, "Error searching Database for '%s'", critere ); }
     ...
     { gchar *tech_id = db->row[0], *acro = db->row[1], *map_text = db->row[2], *libelle = db->row[3];
       ...
       gchar debut[80];
       gint num;
       Info_with_prefix( __func__, THREAD_CLASSE, module->current_thread_tech_id, LOG_DEBUG, "%s: Match found '%s' '%s:%s' - %s", tech_id,
       ...
       if ( sscanf ( map_text, "%[^:]:AO%d", debut, &num ) == 2 )                            /* Découpage de la ligne ev_text */
        { if (1<=num && num<=DMX_CHANNEL)
          ...
          else Info_with_prefix( __func__, THREAD_CLASSE, module->current_thread_tech_id, LOG_WARNING, "map '%s': num %d out of range '%d'",
                         map_text, num, DMX_CHANNEL );
        }
       else Info_with_prefix( __func__, THREAD_CLASSE, module->current_thread_tech_id, LOG_ERR, "event '%s': Sscanf Error", map_text );
     }
    Info_with_prefix( __func__, THREAD_CLASSE, module->current_thread_tech_id, LOG_INFO, "%d AO loaded", cpt );

    Info_with_prefix( __func__, THREAD_CLASSE, module->current_thread_tech_id, LOG_NOTICE, "DMX '%s' : mapping done", tech_id );
  }
/******************************************************************************************************************************/
...
/******************************************************************************************************************************/
 static gboolean Dmx_init ( struct THREAD *module )
  { struct DMX_VARS *vars = module->vars;
    ...
    if (vars->fd<0)
     { Info_with_prefix( __func__, THREAD_CLASSE, module->current_thread_tech_id, LOG_ERR, "%s: Impossible d'ouvrir le device '%s', retour=%d (%s)",
    ...
    Info_with_prefix( __func__, THREAD_CLASSE, module->current_thread_tech_id, LOG_NOTICE, "%s: %s: Ouverture port dmx okay %s",
 ...
 static gboolean Envoyer_trame_dmx_request( struct THREAD *module )
  { struct DMX_VARS *vars = module->vars;
    ...
    if ( write( vars->fd, &vars->Trame_dmx, sizeof(struct TRAME_DMX) ) != sizeof(struct TRAME_DMX) )/* Ecriture de la trame */
     { Info_with_prefix( __func__, THREAD_CLASSE, module->current_thread_tech_id, LOG_ERR, "%s: Write Trame Error '%s'", tech_id, strerror(errno) );
       Dmx_close(module);
       return(FALSE);
     }
    vars->nbr_request++;
          ...
          if (Json_has_member ( request, "token_lvl0" ) && !strcasecmp( Json_get_string ( request, "token_lvl0" ), "SET_AO" ) )
           { gchar *msg_thread_tech_id  = Json_get_string ( request, "token_lvl1" );
             ...
             if (!msg_thread_tech_id)
              ...
              { Info_with_prefix( __func__, THREAD_CLASSE, module->current_thread_tech_id, LOG_ERR, "requete mal formée manque valeur" ); }
             else
              { for (gint num=0; num<DMX_CHANNEL; num++)
                 { if (!strcasecmp( vars->Canal[num].tech_id, msg_thread_tech_id) &&
                    ...
                    { Info_with_prefix( __func__, THREAD_CLASSE, module->current_thread_tech_id, LOG_NOTICE, "Setting %s:%s=%f (Canal %d)",
...
/************************************************* Traitement opérationnel ****************************************************/
       if (module->comm_status == FALSE && vars->date_next_retry <= Partage->top )
        { vars->date_next_retry = 0;
          Info_with_prefix( __func__, THREAD_CLASSE, module->current_thread_tech_id, LOG_NOTICE, "%s: Retrying Connexion.", thread_tech_id );
