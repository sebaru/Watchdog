...
/******************************************************************************************************************************/
 void Http_Init ( void )
  { curl_global_init(CURL_GLOBAL_DEFAULT);
    ...
    Info( __func__, "http", NULL, LOG_DEBUG, "lib cURL initialized" );
  }
/******************************************************************************************************************************/
...
/******************************************************************************************************************************/
 void Http_End ( void )
  { curl_global_cleanup();
    Info( __func__, "http", NULL, LOG_DEBUG, "lib cURL un-initialized" );
  }
/******************************************************************************************************************************/
...
/******************************************************************************************************************************/
 JsonNode *Http_Request ( gchar *url, JsonNode *json_payload, GSList *headers )
  { struct curl_slist *all_headers = NULL;                                                        /* Gestion des headers HTTP */
    ...
    if (!url) return(NULL);
    Info( __func__, "http", NULL, LOG_DEBUG, "Request to %s is starting", url );
/*------------------------------------------------ Init du cURL --------------------------------------------------------------*/
    CURL *curl = curl_easy_init();
    if(!curl) { Info( __func__, "http", NULL, LOG_ERR, "Request to %s: Curl_easy_init failed", url ); return(NULL); }

/*------------------------------------------------ Préparation du payload ----------------------------------------------------*/
    if (json_payload)
     { payload = Json_to_string ( json_payload );
       if (!payload) { Info( __func__, "http", NULL, LOG_ERR, "Request to %s: Json to string failed", url ); goto end; }
     }
...
/*------------------------------------------- Réalisation de la requete ------------------------------------------------------*/
    ...
    Info( __func__, "http", NULL, LOG_DEBUG, "Request to %s: HttpCode = %d", url, http_code );

    if( res != CURLE_OK )
     { Info( __func__, "http", NULL, LOG_ERR, "Request to %s: Curl_easy_perform failed: %s", url, curl_easy_strerror(res) ); }
    else if (http_code == 200)
     { if (buffer->body)
        { ResponseNode = Json_get_from_string ( buffer->body );
          if (!ResponseNode) { Info( __func__, "http", NULL, LOG_ERR, "Request to %s: Response is not json", url ); }
        }
       else Info( __func__, "http", NULL, LOG_ERR, "No body received" );
     }
    else Info( __func__, "http", NULL, LOG_ERR, "Request to %s: HttpCode = %d", url, http_code );
    ...
    else Info( __func__, "http", NULL, LOG_ERR, "Request to %s: ResponseNode Error", url );
...
/********************************************************* Envoi de la requete ************************************************/
    ...
    Info( __func__, "http", NULL, LOG_DEBUG, "Sending to API %s", query );
    ...
    Info( __func__, "http", NULL, LOG_DEBUG, "%s: Status %d", query, http_code );

    if (http_code!=200)
     { Info( __func__, "http", NULL, LOG_ERR, "%s Error %d", query, http_code ); }
    return(ResponseNode);
 }
/******************************************************************************************************************************/
...
/********************************************************* Envoi de la requete ************************************************/
    ...
    if (!ResponseNode) { Info( __func__, "http", NULL, LOG_ERR, "Error with Http_Get %s", query ); return(NULL); }
    gint http_code = Json_get_int ( ResponseNode, "http_code" );
    Info( __func__, "http", NULL, LOG_DEBUG, "%s Status %d for '%s'", URI, http_code, query );

    if (http_code!=200)
     { Info( __func__, "http", NULL, LOG_ERR, "%s Error %d for '%s'", URI, http_code, query );
       Json_unref ( ResponseNode );
       gchar *nom_fichier = Http_Query_to_cache ( query );
       if (nom_fichier)
        { ResponseNode = Json_read_from_file ( nom_fichier );
          g_free(nom_fichier);
          if (ResponseNode) Info( __func__, "http", NULL, LOG_INFO, "Using cache for %s OK", query );
                       else Info( __func__, "http", NULL, LOG_ERR,  "Using cache for %s failed", query );
        } else Info( __func__, "http", NULL, LOG_ERR, "Cache error for %s", query );
     }
    else
