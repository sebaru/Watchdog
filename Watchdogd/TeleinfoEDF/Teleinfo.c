...
/******************************************************************************************************************************/
 static int Init_teleinfo ( struct THREAD *module )
  { struct termios oldtio;
    ...
    if (fd<0)
    { Info( __func__, "teleinfo", thread_tech_id, LOG_ERR,
    ...
    Info( __func__, "teleinfo", thread_tech_id, LOG_NOTICE, "Ouverture port teleinfo okay %s", port );
    return(fd);
  }
/******************************************************************************************************************************/
        ...
        { pthread_mutex_lock ( &module->synchro );
          ...
          if (Json_has_member ( message, "token_lvl0" ))
           { gchar *token_lvl0 = Json_get_string ( message, "token_lvl0" );
             Info( __func__, "teleinfo", thread_tech_id, LOG_DEBUG, "%s: token_lvl0 '%s' not for this thread", thread_tech_id, token_lvl0 );
           }
          Json_unref(message);
        }
       ...
       if (vars->mode == TINFO_WAIT_BEFORE_RETRY)
        { if ( vars->date_next_retry <= Partage->top )
           { vars->mode = TINFO_RETRING;
             ...
             Info( __func__, "teleinfo", thread_tech_id, LOG_NOTICE, "%s: Retrying Connexion.", thread_tech_id );
           }
        }
        ...
        { vars->fd = Init_teleinfo( module );
          if (vars->fd<0)                                                               /* On valide l'acces aux ports */
           ...
           { vars->mode = TINFO_CONNECTED;
             Info( __func__, "teleinfo", thread_tech_id, LOG_INFO, "%s: Acces TELEINFO FD=%d", thread_tech_id, vars->fd );
           }
        }
