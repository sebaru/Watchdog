...
#ifdef bouh
/******************************************************************************************************************************/
/* Modbus_mode_to_string: Convertit le mode phidget (int) en sa version chaine de caractere                                    */
/* Entrée : le module_phidget                                                                                                  */
/* Sortie : char *mode_char                                                                                                   */
/******************************************************************************************************************************/
 static gchar *Modbus_mode_to_string ( struct MODULE_PHIDGET *module )
  { static gchar chaine[32];
    ...
    switch ( module->mode )
     {
       case PHIDGET_GET_DESCRIPTION : return("Get_Description");
       case PHIDGET_GET_FIRMWARE    : return("Get_firmware   ");
       case PHIDGET_INIT_WATCHDOG1  : return("Init_Watchdog_1");
       case PHIDGET_INIT_WATCHDOG2  : return("Init_Watchdog_2");
       case PHIDGET_INIT_WATCHDOG3  : return("Init_Watchdog_3");
       case PHIDGET_INIT_WATCHDOG4  : return("Init_Watchdog_4");
       case PHIDGET_GET_NBR_AI      : return("Init_Get_Nbr_AI");
       case PHIDGET_GET_NBR_AO      : return("Init_Get_Nbr_AO");
       case PHIDGET_GET_NBR_DI      : return("Init_Get_Nbr_DI");
       case PHIDGET_GET_NBR_DO      : return("Init_Get_Nbr_DO");
       case PHIDGET_GET_DI          : return("Get DI State   ");
       case PHIDGET_GET_AI          : return("Get AI State   ");
       case PHIDGET_SET_DO          : return("Set DO State   ");
       case PHIDGET_SET_AO          : return("Set AO State   ");
...
/*     { Json_add_int ( builder, "nbr_request_par_sec", Cfg_phidget.nbr_request_par_sec ); }*/
     ...
     { pthread_mutex_lock( &Lib->synchro );
       ...
       while ( liste_modules )
        { struct MODULE_PHIDGET *module = liste_modules->data;
          ...
          Json_add_string ( builder, "tech_id", module->phidget.tech_id );
...
/* Sortie : FALSE si pb                                                                                                       */
/******************************************************************************************************************************/
 static void Admin_json_phidget_list ( struct LIBRAIRIE *Lib, SoupMessage *msg )
  { JsonBuilder *builder;
...
/************************************************ Préparation du buffer JSON **************************************************/
    ...
    Json_add_bool ( builder, "thread_is_running", Lib->Thread_run );
    g_snprintf(chaine, sizeof(chaine), "SELECT * FROM phidget_modules" );
...
/* Sortie : FALSE si pb                                                                                                       */
/******************************************************************************************************************************/
 static void Admin_json_phidget_del ( struct LIBRAIRIE *Lib, SoupMessage *msg )
  { GBytes *request_brute;
    ...
    g_snprintf( chaine, sizeof(chaine), "DELETE FROM phidget_modules WHERE tech_id='%s'", tech_id );
    g_free(tech_id);
    SQL_Write ( chaine );
    soup_message_set_status (msg, SOUP_STATUS_OK);
...
/* Sortie : néant                                                                                                             */
/******************************************************************************************************************************/
 static void Admin_json_phidget_set ( struct LIBRAIRIE *Lib, SoupMessage *msg )
  { GBytes *request_brute;
    gchar requete[256];
    gsize taille;
    ...
    g_snprintf( requete, sizeof(requete),
               "UPDATE phidget_modules SET description='%s', hostname='%s', watchdog='%d', max_request_par_sec='%d' WHERE tech_id='%s'",
...
/* Sortie : néant                                                                                                             */
/******************************************************************************************************************************/
 static void Admin_json_phidget_start ( struct LIBRAIRIE *Lib, SoupMessage *msg, gboolean start )
  { GBytes *request_brute;
    gchar requete[256];
    gsize taille;
    ...
    g_snprintf( requete, sizeof(requete), "UPDATE phidget_modules SET enable='%d' WHERE tech_id='%s'", (start ? 1 : 0), tech_id );
    if (SQL_Write (requete))
     { soup_message_set_status (msg, SOUP_STATUS_OK);
       Lib->Thread_reload = TRUE;
     }
...
/******************************************************************************************************************************/
/* Admin_json_phidget_set: Ajoute un composant WAGO dans système                                                               */
/* Entrées: la connexion Websocket                                                                                            */
/* Sortie : néant                                                                                                             */
/******************************************************************************************************************************/
 static void Admin_json_phidget_add ( struct LIBRAIRIE *Lib, SoupMessage *msg )
  { GBytes *request_brute;
    gchar requete[256];
    gsize taille;
    ...
    g_snprintf( requete, sizeof(requete),
               "INSERT INTO phidget_modules SET tech_id='%s', description='%s', hostname='%s', watchdog='%d', "
  ...
  { if (access_level < 6)
     { soup_message_set_status_full (msg, SOUP_STATUS_FORBIDDEN, "Pas assez de privileges");
       return;
         ...
         if (!strcasecmp(path, "/list"))     { Admin_json_phidget_list ( lib, msg ); }
    else if (!strcasecmp(path, "/modules_status")) { Admin_json_phidget_modules_status ( lib, msg ); }
    else if (!strcasecmp(path, "/status"))   { Admin_json_phidget_status ( lib, msg ); }
    else if (!strcasecmp(path, "/del"))      { Admin_json_phidget_del ( lib, msg ); }
    else if (!strcasecmp(path, "/set"))      { Admin_json_phidget_set ( lib, msg ); }
    else if (!strcasecmp(path, "/add"))      { Admin_json_phidget_add ( lib, msg ); }
    else if (!strcasecmp(path, "/start"))    { Admin_json_phidget_start ( lib, msg, TRUE ); }
    else if (!strcasecmp(path, "/stop"))     { Admin_json_phidget_start ( lib, msg, FALSE ); }
