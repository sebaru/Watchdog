...
/* Sortie: une GList                                                                                                          */
/******************************************************************************************************************************/
 gboolean Recuperer_phidgetDB ( struct DB *db )
  { gchar requete[256];

    g_snprintf( requete, sizeof(requete),                                                                      /* Requete SQL */
                "SELECT id,date_create,enable,hostname,tech_id,description "
                " FROM phidget_modules ORDER BY description" );
...
/* Sortie: une GList                                                                                                          */
/******************************************************************************************************************************/
 struct PHIDGETDB *Recuperer_phidgetDB_suite( struct DB *db )
  { struct PHIDGETDB *phidget;
    ...
    phidget = (struct PHIDGETDB *)g_try_malloc0( sizeof(struct PHIDGETDB) );
    if (!phidget) Info_new( Config.log, Cfg_phidget.lib->Thread_debug, LOG_ERR,
                          "%s: Erreur allocation mémoire", __func__ );
    else
     { g_snprintf( phidget->description, sizeof(phidget->description), "%s", db->row[6] );
       g_snprintf( phidget->tech_id,     sizeof(phidget->tech_id),     "%s", db->row[4] );
       g_snprintf( phidget->hostname,    sizeof(phidget->hostname),    "%s", db->row[3] );
       g_snprintf( phidget->date_create, sizeof(phidget->date_create), "%s", db->row[1] );
       phidget->id                  = atoi(db->row[0]);
       phidget->enable              = atoi(db->row[2]);
       phidget->watchdog            = atoi(db->row[5]);
       phidget->max_request_par_sec = atoi(db->row[7]);
     }
    return(phidget);
  }
/******************************************************************************************************************************/
...
/* Sortie: néant                                                                                                              */
/******************************************************************************************************************************/
 static void Deconnecter_module ( struct MODULE_PHIDGET *module )
  { if (!module) return;
    ...
    module->date_retente = Partage->top + PHIDGET_RETRY;
    if (module->DI) g_free(module->DI);
    if (module->AI) g_free(module->AI);
    if (module->DO) g_free(module->DO);
    Dls_data_set_WATCHDOG ( NULL, module->phidget.tech_id, "IO_COMM", &module->bit_comm, 0 );
    Info_new( Config.log, Cfg_phidget.lib->Thread_debug, LOG_INFO, "%s: '%s': Module disconnected", __func__, module->phidget.tech_id );
  }
/******************************************************************************************************************************/
...
/******************************************************************************************************************************/
 static gboolean Connecter_module ( struct MODULE_PHIDGET *module )
  { struct addrinfo *result, *rp;
    ...
    Info_new( Config.log, Cfg_phidget.lib->Thread_debug, LOG_DEBUG, "%s: '%s' : Trying to connect module %d to %s", __func__,
              module->phidget.tech_id, module->phidget.id, module->phidget.hostname );

    s = getaddrinfo( module->phidget.hostname, "502", &hints, &result);
    if (s != 0)
     { Info_new( Config.log, Cfg_phidget.lib->Thread_debug, LOG_WARNING,
                "%s: '%s': getaddrinfo Failed for module %d (%s) (%s)", __func__, module->phidget.tech_id,
                 module->phidget.id, module->phidget.hostname, gai_strerror(s) );
       return(FALSE);
     }
   ...
   /* getaddrinfo() returns a list of address structures.
    ...
    for (rp = result; rp != NULL; rp = rp->ai_next)
     { connexion = socket(rp->ai_family, rp->ai_socktype, rp->ai_protocol);
       if (connexion == -1)
        { Info_new( Config.log, Cfg_phidget.lib->Thread_debug, LOG_WARNING,
                   "%s: '%s': Socket creation failed for phidget %d (%s)", __func__, module->phidget.tech_id,
                    module->phidget.id, module->phidget.hostname );
          continue;
        }

       if ( setsockopt ( connexion, SOL_SOCKET, SO_SNDTIMEO, (char *)&sndtimeout, sizeof(sndtimeout)) < 0 )
        { Info_new( Config.log, Cfg_phidget.lib->Thread_debug, LOG_WARNING,
                   "%s: '%s': Socket Set Options failed for phidget %d (%s)", __func__, module->phidget.tech_id,
                    module->phidget.id, module->phidget.hostname );
          continue;
        }
       ...
       if (connect(connexion, rp->ai_addr, rp->ai_addrlen) != -1)
        { Info_new( Config.log, Cfg_phidget.lib->Thread_debug, LOG_INFO,
                   "%s: '%s': %d (%s) family=%d", __func__, module->phidget.tech_id,
                    module->phidget.id, module->phidget.hostname, rp->ai_family );

          break;  /* Success */
        }
       else
        { Info_new( Config.log, Cfg_phidget.lib->Thread_debug, LOG_NOTICE,
                   "%s: '%s': connexion refused by module %d (%s) family=%d error '%s'", __func__, module->phidget.tech_id,
                    module->phidget.id, module->phidget.hostname, rp->ai_family, strerror(errno) );
        }
    ...
    module->started = TRUE;
    module->mode = PHIDGET_GET_DESCRIPTION;
    module->DI = NULL;
    module->AI = NULL;
    module->DO = NULL;
    Info_new( Config.log, Cfg_phidget.lib->Thread_debug, LOG_INFO, "%s: '%s': Module Connected", __func__, module->phidget.tech_id );
...
/******************************************************************************************************************************/
 static void Interroger_description( struct MODULE_PHIDGET *module )
  { struct TRAME_PHIDGET_REQUETE requete;                                                     /* Definition d'une trame PHIDGET */
    gint retour;
    ...
    if ( retour != 12 )                                                                                /* Envoi de la requete */
     { Info_new( Config.log, Cfg_phidget.lib->Thread_debug, LOG_WARNING,
               "%s: '%s': failed for module %d (%s): error %d", __func__, module->phidget.tech_id,
               module->phidget.id, module->phidget.hostname, retour );
       Deconnecter_module( module );
     }
    else
     { Info_new( Config.log, Cfg_phidget.lib->Thread_debug, LOG_DEBUG, "%s: '%s': OK for %d",
                 __func__, module->phidget.tech_id, module->phidget.id );
...
/* Entrée: L'id de la transmission, et la trame a transmettre                                                                 */
/******************************************************************************************************************************/
 static void Interroger_firmware( struct MODULE_PHIDGET *module )
  { struct TRAME_PHIDGET_REQUETE requete;                                                     /* Definition d'une trame PHIDGET */
    gint retour;
...
/* Sortie: néant                                                                                                              */
/******************************************************************************************************************************/
 static void Init_watchdog1( struct MODULE_PHIDGET *module )
  { struct TRAME_PHIDGET_REQUETE requete;                                                     /* Definition d'une trame PHIDGET */
    gint retour;
    ...
    if ( retour != 12 )                                                                                /* Envoi de la requete */
     { Info_new( Config.log, Cfg_phidget.lib->Thread_debug, LOG_WARNING,
               "%s: '%s': 'stop watchdog failed' for %d (error %d)", __func__, module->phidget.tech_id, module->phidget.id, retour );
       Deconnecter_module( module );
     }
    else
     { Info_new( Config.log, Cfg_phidget.lib->Thread_debug, LOG_DEBUG,
               "%s: '%s': 'stop watchdog OK' for %d", __func__, module->phidget.tech_id, module->phidget.id );
       module->request = TRUE;                                                                    /* Une requete a élé lancée */
     }
  }
/******************************************************************************************************************************/
...
/* Sortie: ?                                                                                                                  */
/******************************************************************************************************************************/
 static void Init_watchdog2( struct MODULE_PHIDGET *module )
  { struct TRAME_PHIDGET_REQUETE requete;                                                     /* Definition d'une trame PHIDGET */
    gint retour;
    ...
    requete.unit_id        = 0x00;                                                                                    /* 0xFF */
    requete.fct            = MBUS_WRITE_REGISTER;
    requete.adresse        = htons( 0x1009 );                                   /* Close PHIDGET socket after watchdog timeout */
    requete.valeur         = htons( 0x0001 );

    retour = write ( module->connexion, &requete, 12 );
    if ( retour != 12 )                                                                                /* Envoi de la requete */
     { Info_new( Config.log, Cfg_phidget.lib->Thread_debug, LOG_WARNING,
               "%s: '%s': 'close phidget tcp on watchdog' failed for %d (error %d)", __func__, module->phidget.tech_id,
               module->phidget.id, retour );
       Deconnecter_module( module );
     }
    else
     { Info_new( Config.log, Cfg_phidget.lib->Thread_debug, LOG_DEBUG,
                "%s: '%s': 'close phidget tcp on watchdog' OK for %d", __func__, module->phidget.tech_id, module->phidget.id );
       module->request = TRUE;                                                /* Une requete a élé lancée */
     }
  }
/******************************************************************************************************************************/
...
/******************************************************************************************************************************/
 static void Init_watchdog3( struct MODULE_PHIDGET *module )
  { struct TRAME_PHIDGET_REQUETE requete;                                                     /* Definition d'une trame PHIDGET */
    gint retour;
    ...
    requete.valeur         = htons( module->phidget.watchdog );                     /* coupure sortie, en 100ième de secondes  */

    retour = write ( module->connexion, &requete, 12 );
    if ( retour != 12 )                                                                                /* Envoi de la requete */
     { Info_new( Config.log, Cfg_phidget.lib->Thread_debug, LOG_WARNING,
               "%s: '%s': 'init watchdog timer' failed for %d (error %d)", __func__, module->phidget.tech_id,
               module->phidget.id, retour );
       Deconnecter_module( module );
     }
    else
     { Info_new( Config.log, Cfg_phidget.lib->Thread_debug, LOG_DEBUG,
                "%s: '%s': 'init watchdog timer' OK for %d", __func__, module->phidget.tech_id, module->phidget.id );
       module->request = TRUE;                                                /* Une requete a élé lancée */
     }
  }
/******************************************************************************************************************************/
...
/* Sortie: ?                                                                                                                  */
/******************************************************************************************************************************/
 static void Init_watchdog4( struct MODULE_PHIDGET *module )
  { struct TRAME_PHIDGET_REQUETE requete;                                                     /* Definition d'une trame PHIDGET */
    gint retour;
    ...
    if ( retour != 12 )                                                                                /* Envoi de la requete */
     { Info_new( Config.log, Cfg_phidget.lib->Thread_debug, LOG_WARNING,
                "%s: '%s': 'watchdog start' failed for %d (error %d)", __func__, module->phidget.tech_id,
                 module->phidget.id, retour );
       Deconnecter_module( module );
     }
    else
     { Info_new( Config.log, Cfg_phidget.lib->Thread_debug, LOG_DEBUG,
                "%s: '%s': Init_watchdog_phidget: 'watchdog start' OK for %d", __func__, module->phidget.tech_id, module->phidget.id );
       module->request = TRUE;                                                /* Une requete a élé lancée */
     }
  }
/******************************************************************************************************************************/
...
/* Entrée: L'id de la transmission, et la trame a transmettre                                                                 */
/******************************************************************************************************************************/
 static void Interroger_nbr_entree_ANA( struct MODULE_PHIDGET *module )
  { struct TRAME_PHIDGET_REQUETE requete;                                                     /* Definition d'une trame PHIDGET */
    gint retour;
    ...
    if ( retour != 12 )                                                                                /* Envoi de la requete */
     { Info_new( Config.log, Cfg_phidget.lib->Thread_debug, LOG_WARNING,
               "%s: '%s': failed for %d (error %d)", __func__, module->phidget.tech_id,
                module->phidget.id, retour );
       Deconnecter_module( module );
     }
    else
     { Info_new( Config.log, Cfg_phidget.lib->Thread_debug, LOG_DEBUG,
                "%s: '%s': OK for %d", __func__, module->phidget.tech_id, module->phidget.id );
       module->request = TRUE;                                                                    /* Une requete a élé lancée */
     }
  }
/******************************************************************************************************************************/
...
/* Entrée: L'id de la transmission, et la trame a transmettre                                                                 */
/******************************************************************************************************************************/
 static void Interroger_nbr_sortie_ANA( struct MODULE_PHIDGET *module )
  { struct TRAME_PHIDGET_REQUETE requete;                                                     /* Definition d'une trame PHIDGET */
    gint retour;
    ...
    if ( retour != 12 )                                                                                /* Envoi de la requete */
     { Info_new( Config.log, Cfg_phidget.lib->Thread_debug, LOG_WARNING,
               "%s: '%s': failed %d (error %d)", __func__, module->phidget.tech_id,
                module->phidget.id, retour );
       Deconnecter_module( module );
     }
    else
     { Info_new( Config.log, Cfg_phidget.lib->Thread_debug, LOG_DEBUG,
               "%s: '%s': OK", __func__, module->phidget.tech_id, module->phidget.id );
       module->request = TRUE;                                                                    /* Une requete a élé lancée */
     }
  }
/******************************************************************************************************************************/
...
/* Entrée: L'id de la transmission, et la trame a transmettre                                                                 */
/******************************************************************************************************************************/
 static void Interroger_nbr_entree_TOR( struct MODULE_PHIDGET *module )
  { struct TRAME_PHIDGET_REQUETE requete;                                                     /* Definition d'une trame PHIDGET */
    gint retour;
    ...
    if ( retour != 12 )                                                                                /* Envoi de la requete */
     { Info_new( Config.log, Cfg_phidget.lib->Thread_debug, LOG_WARNING,
               "%s: '%s': failed %d (error %d - '%s')", __func__, module->phidget.tech_id, module->phidget.id, retour, strerror(errno) );
       Deconnecter_module( module );
     }
    else
     { Info_new( Config.log, Cfg_phidget.lib->Thread_debug, LOG_DEBUG,
...
/* Entrée: L'id de la transmission, et la trame a transmettre                                                                 */
/******************************************************************************************************************************/
 static void Interroger_nbr_sortie_TOR( struct MODULE_PHIDGET *module )
  { struct TRAME_PHIDGET_REQUETE requete;                                                     /* Definition d'une trame PHIDGET */
    gint retour;
    ...
    if ( retour != 12 )                                                                                /* Envoi de la requete */
     { Info_new( Config.log, Cfg_phidget.lib->Thread_debug, LOG_WARNING,
                 "%s: '%s': write error Module %d (error %d, '%s')", __func__, module->phidget.tech_id,
                 module->phidget.id, retour, strerror(errno) );
       Deconnecter_module( module );
     }
    else
     { Info_new( Config.log, Cfg_phidget.lib->Thread_debug, LOG_DEBUG,
...
/* Sortie: ?                                                                                                                  */
/******************************************************************************************************************************/
 static void Interroger_entree_tor( struct MODULE_PHIDGET *module )
  { struct TRAME_PHIDGET_REQUETE requete;                                                     /* Definition d'une trame PHIDGET */
...
/* Sortie: ?                                                                                                                  */
/******************************************************************************************************************************/
 static void Interroger_entree_ana( struct MODULE_PHIDGET *module )
  { struct TRAME_PHIDGET_REQUETE requete;                                                     /* Definition d'une trame PHIDGET */
...
/* Sortie: ?                                                                                                                  */
/******************************************************************************************************************************/
 static void Interroger_sortie_tor( struct MODULE_PHIDGET *module )
  { struct TRAME_PHIDGET_REQUETE requete;                                                     /* Definition d'une trame PHIDGET */
...
/* Sortie: néant                                                                                                              */
/******************************************************************************************************************************/
 static void Interroger_sortie_ana( struct MODULE_PHIDGET *module )
  { struct TRAME_PHIDGET_REQUETE requete;                                                     /* Definition d'une trame PHIDGET */
...
/******************************************************************************************************************************/
/* Phidget_do_mapping : mappe les entrees/sorties Wago avec la zone de mémoire interne dynamique                               */
/* Entrée : la structure referencant le module                                                                                */
/* Sortie : rien                                                                                                              */
/******************************************************************************************************************************/
 static void Phidget_do_mapping ( struct MODULE_PHIDGET *module )
  { gchar critere[80];
    struct DB *db;

    module->AI = g_try_malloc0( sizeof(gpointer) * module->nbr_entree_ana );
    if (!module->AI)
     { Info_new( Config.log, Cfg_phidget.lib->Thread_debug, LOG_ERR, "%s: '%s': Memory Error for AI", __func__, module->phidget.tech_id );
       return;
     } else Info_new( Config.log, Cfg_phidget.lib->Thread_debug, LOG_INFO,
                      "%s: '%s': Allocated %d AI", __func__, module->phidget.tech_id, module->nbr_entree_ana );

    module->DI = g_try_malloc0( sizeof(gpointer) * module->nbr_entree_tor );
    if (!module->DI)
     { Info_new( Config.log, Cfg_phidget.lib->Thread_debug, LOG_ERR, "%s: '%s': Memory Error for DI", __func__ , module->phidget.tech_id);
       return;
     } else Info_new( Config.log, Cfg_phidget.lib->Thread_debug, LOG_INFO,
                      "%s: '%s': Allocated %d DI", __func__, module->phidget.tech_id, module->nbr_entree_tor );

    module->DO = g_try_malloc0( sizeof(gpointer) * module->nbr_sortie_tor );
    if (!module->DO)
     { Info_new( Config.log, Cfg_phidget.lib->Thread_debug, LOG_ERR, "%s: '%s': Memory Error for DO", __func__, module->phidget.tech_id );
       return;
     } else Info_new( Config.log, Cfg_phidget.lib->Thread_debug, LOG_INFO,
                      "%s: '%s': Allocated %d DO", __func__, module->phidget.tech_id, module->nbr_sortie_tor );

/******************************* Recherche des event text EA a raccrocher aux bits internes ***********************************/
    if ( ! Recuperer_mnemos_AI_by_tag ( &db, module->phidget.tech_id, "AI%%" ) )
     { Info_new( Config.log, Cfg_phidget.lib->Thread_debug, LOG_ERR, "%s: '%s': Error searching Database for '%s'",
                 __func__, module->phidget.tech_id, critere );
     }
    else while ( Recuperer_mnemos_AI_suite( &db ) )
     { gchar *tech_id = db->row[0], *acro = db->row[1], *map_tag = db->row[2], *libelle = db->row[3];
       gint num;
       Info_new( Config.log, Cfg_phidget.lib->Thread_debug, LOG_INFO, "%s: '%s': Map found '%s' '%s:%s' - %s",
                 __func__, module->phidget.tech_id, map_tag, tech_id, acro, libelle );
       if ( sscanf ( map_tag, "AI%d", &num ) == 1 )                                          /* Découpage de la ligne ev_text */
        { if (num<module->nbr_entree_ana)
          ...
          else Info_new( Config.log, Cfg_phidget.lib->Thread_debug, LOG_WARNING, "%s: '%s': map '%s': num %d out of range '%d'",
                         __func__, module->phidget.tech_id, map_tag, num, module->nbr_entree_ana );
        }
       else Info_new( Config.log, Cfg_phidget.lib->Thread_debug, LOG_ERR, "%s: '%s': event '%s': Sscanf Error",
                      __func__, module->phidget.tech_id, map_tag );
     }
/******************************* Recherche des event text EA a raccrocher aux bits internes ***********************************/
    if ( ! Recuperer_mnemos_DI_by_tag ( &db, module->phidget.tech_id, "DI%%" ) )
     { Info_new( Config.log, Cfg_phidget.lib->Thread_debug, LOG_ERR, "%s: '%s': Error searching Database for '%s'",
                 __func__, module->phidget.tech_id, critere );
     }
        ...
        { if (num<module->nbr_entree_tor)
           { Dls_data_get_DI ( tech_id, acro, &module->DI[num] );        /* bit déjà existant deja dans la structure DLS DATA */
             if(module->DI[num] == NULL) Dls_data_set_DI ( NULL, tech_id, acro, &module->DI[num], FALSE );/* Sinon, on le crée */
                         ...
                         __func__, module->phidget.tech_id, map_tag, num, module->nbr_entree_tor );
        }
       else Info_new( Config.log, Cfg_phidget.lib->Thread_debug, LOG_ERR, "%s: '%s': event '%s': Sscanf Error",
                      __func__, module->phidget.tech_id, map_tag );
     }
/*********************************** Recherche des events DO a raccrocher aux bits internes ***********************************/
    if ( ! Recuperer_mnemos_DO_by_tag ( &db, module->phidget.tech_id, "DO%%" ) )
     { Info_new( Config.log, Cfg_phidget.lib->Thread_debug, LOG_ERR, "%s: '%s': Error searching Database for '%s'",
                 __func__, module->phidget.tech_id, critere );
     }
        ...
        { if (num<module->nbr_sortie_tor)
           { Dls_data_get_DO ( tech_id, acro, &module->DO[num] );        /* bit déjà existant deja dans la structure DLS DATA */
             if(module->DO[num] == NULL) Dls_data_set_DO ( NULL, tech_id, acro, &module->DO[num], FALSE );     /* Sinon, on le crée */
                         ...
                         __func__, module->phidget.tech_id, map_tag, num, module->nbr_sortie_tor );
        }
       else Info_new( Config.log, Cfg_phidget.lib->Thread_debug, LOG_ERR, "%s: '%s': event '%s': Sscanf Error",
                      __func__, module->phidget.tech_id, map_tag );
     }
/******************************* Recherche des event text EA a raccrocher aux bits internes ***********************************/
    Info_new( Config.log, Cfg_phidget.lib->Thread_debug, LOG_NOTICE, "%s: '%s': Module '%s' : mapping done",
              __func__, module->phidget.tech_id, module->phidget.description );
  }
/******************************************************************************************************************************/
/* Recuperer_borne: Recupere les informations d'une borne PHIDGET                                                              */
/* Entrée: identifiants des modules et borne                                                                                  */
/* Sortie: ?                                                                                                                  */
/******************************************************************************************************************************/
 static void Phidget_Processer_trame( struct MODULE_PHIDGET *module )
  { module->nbr_oct_lu = 0;
    module->request = FALSE;                                                                     /* Une requete a été traitée */

    if ( (guint16) module->response.proto_id )
     { Info_new( Config.log, Cfg_phidget.lib->Thread_debug, LOG_WARNING, "%s: '%s': wrong proto_id", __func__, module->phidget.tech_id );
       Deconnecter_module( module );
     }
    else
     { int cpt_byte, cpt_poid, cpt;
       module->date_last_reponse = Partage->top;                                                   /* Estampillage de la date */
       Dls_data_set_WATCHDOG ( NULL, module->phidget.tech_id, "IO_COMM", &module->bit_comm, 600 );
       if (ntohs(module->response.transaction_id) != module->transaction_id)                              /* Mauvaise reponse */
        { Info_new( Config.log, Cfg_phidget.lib->Thread_debug, LOG_WARNING,
                   "%s: '%s': wrong transaction_id for module %d  attendu %d, recu %d", __func__, module->phidget.tech_id,
                    module->phidget.id, module->transaction_id, ntohs(module->response.transaction_id) );
        }
       if ( module->response.fct >=0x80 )
        { Info_new( Config.log, Cfg_phidget.lib->Thread_debug, LOG_WARNING,
                   "%s: '%s': Erreur Reponse Module %d, Error %d, Exception code %d", __func__, module->phidget.tech_id,
                    module->phidget.id, module->response.fct, (int)module->response.data[0] );
          Deconnecter_module( module );
        }
        ...
        { case PHIDGET_GET_DI:
          ...
          case PHIDGET_GET_AI:
               ...
               module->mode = PHIDGET_SET_DO;
               break;
          case PHIDGET_SET_DO:
               module->mode = PHIDGET_SET_AO;
               break;
          case PHIDGET_SET_AO:
               module->mode = PHIDGET_GET_DI;
               break;
          case PHIDGET_GET_DESCRIPTION:
             { gchar chaine[32];
               gint taille;
               ...
               chaine[taille] = 0;
               Info_new( Config.log, Cfg_phidget.lib->Thread_debug, LOG_INFO, "%s: '%s': Module %d Get Description (size %d) '%s'",
                         __func__, module->phidget.tech_id, module->phidget.id, taille, chaine );
               module->mode = PHIDGET_GET_FIRMWARE;
               break;
            }
          case PHIDGET_GET_FIRMWARE:
             { gchar chaine[64];
               gint taille;
               memset ( chaine, 0, sizeof(chaine) );
               taille = module->response.data[0];
               ...
               chaine[2] = ntohs( (gint16)module->response.data[3] );
               chaine[taille] = 0;
               Info_new( Config.log, Cfg_phidget.lib->Thread_debug, LOG_INFO, "%s: '%s': Module %d Get Firmware (size %d) %s",
                         __func__, module->phidget.tech_id, module->phidget.id, taille, chaine );
               module->mode = PHIDGET_INIT_WATCHDOG1;
               break;
            }
          case PHIDGET_INIT_WATCHDOG1:
               Info_new( Config.log, Cfg_phidget.lib->Thread_debug, LOG_DEBUG, "%s: '%s': Module %d Watchdog1 = %d %d",
                         __func__, module->phidget.tech_id, module->phidget.id,
                         ntohs( *(gint16 *)((gchar *)&module->response.data + 0) ),
                         ntohs( *(gint16 *)((gchar *)&module->response.data + 2) )
                       );
               module->mode = PHIDGET_INIT_WATCHDOG2;
               break;
          case PHIDGET_INIT_WATCHDOG2:
               Info_new( Config.log, Cfg_phidget.lib->Thread_debug, LOG_DEBUG, "%s: '%s': Module %d Watchdog2 = %d %d",
                         __func__, module->phidget.tech_id, module->phidget.id,
                         ntohs( *(gint16 *)((gchar *)&module->response.data + 0) ),
                         ntohs( *(gint16 *)((gchar *)&module->response.data + 2) )
                       );
               module->mode = PHIDGET_INIT_WATCHDOG3;
               break;
          case PHIDGET_INIT_WATCHDOG3:
               Info_new( Config.log, Cfg_phidget.lib->Thread_debug, LOG_DEBUG, "%s: '%s': Module %d Watchdog3 = %d %d",
                         __func__, module->phidget.tech_id, module->phidget.id,
                         ntohs( *(gint16 *)((gchar *)&module->response.data + 0) ),
                         ntohs( *(gint16 *)((gchar *)&module->response.data + 2) )
                       );
               module->mode = PHIDGET_INIT_WATCHDOG4;
               break;
          case PHIDGET_INIT_WATCHDOG4:
               Info_new( Config.log, Cfg_phidget.lib->Thread_debug, LOG_DEBUG, "%s: '%s': Module %d Watchdog4 = %d %d",
                         __func__, module->phidget.tech_id, module->phidget.id,
                         ntohs( *(gint16 *)((gchar *)&module->response.data + 0) ),
                         ntohs( *(gint16 *)((gchar *)&module->response.data + 2) )
                       );
               module->mode = PHIDGET_GET_NBR_AI;
               break;
          case PHIDGET_GET_NBR_AI:
               module->nbr_entree_ana = ntohs( *(gint16 *)((gchar *)&module->response.data + 1) ) / 16;
               Info_new( Config.log, Cfg_phidget.lib->Thread_debug, LOG_INFO, "%s: '%s': Module %d Get number Entree ANA = %d",
                         __func__, module->phidget.tech_id, module->phidget.id, module->nbr_entree_ana
                       );
               module->mode = PHIDGET_GET_NBR_AO;
               break;
          case PHIDGET_GET_NBR_AO:
               module->nbr_sortie_ana = ntohs( *(gint16 *)((gchar *)&module->response.data + 1) ) / 16;
               Info_new( Config.log, Cfg_phidget.lib->Thread_debug, LOG_INFO, "%s: '%s': Module %d Get number Sortie ANA = %d",
                         __func__, module->phidget.tech_id, module->phidget.id, module->nbr_sortie_ana
                       );
               module->mode = PHIDGET_GET_NBR_DI;
               break;
          case PHIDGET_GET_NBR_DI:
                { gint nbr;
                  nbr = ntohs( *(gint16 *)((gchar *)&module->response.data + 1) );
                  module->nbr_entree_tor = nbr;
                  Info_new( Config.log, Cfg_phidget.lib->Thread_debug, LOG_INFO, "%s: '%s': Module %d Get number Entree TOR = %d",
                            __func__, module->phidget.tech_id, module->phidget.id, module->nbr_entree_tor );
                  module->mode = PHIDGET_GET_NBR_DO;
                }
               break;
          case PHIDGET_GET_NBR_DO:
               module->nbr_sortie_tor = ntohs( *(gint16 *)((gchar *)&module->response.data + 1) );
               Info_new( Config.log, Cfg_phidget.lib->Thread_debug, LOG_INFO, "%s: '%s': Module %d Get number Sortie TOR = %d",
                         __func__, module->phidget.tech_id, module->phidget.id, module->nbr_sortie_tor );
               module->mode = PHIDGET_GET_DI;
               Phidget_do_mapping( module );                                        /* Initialise le mapping des I/O du module */
               break;
        }
     }
    memset (&module->response, 0, sizeof(struct TRAME_PHIDGET_REPONSE) );
  }
/******************************************************************************************************************************/
/* Recuperer_borne: Recupere les informations d'une borne PHIDGET                                                              */
/* Entrée: identifiants des modules et borne                                                                                  */
/* Sortie: ?                                                                                                                  */
/******************************************************************************************************************************/
 static void Recuperer_reponse_module( struct MODULE_PHIDGET *module )
  { fd_set fdselect;
    struct timeval tv;
    gint retval, cpt;
    ...
    if (module->date_last_reponse + 600 < Partage->top)                                      /* Detection attente trop longue */
     { Info_new( Config.log, Cfg_phidget.lib->Thread_debug, LOG_WARNING,
                ...
                "transactionID=%06d, nbr_deconnect=%02d, last_reponse=%03ds ago, retente=in %03ds, date_next_eana=in %03ds",
                 __func__, module->phidget.tech_id, module->phidget.id, module->phidget.enable, module->started, module->mode,
    ...
    if ( retval>0 && FD_ISSET(module->connexion, &fdselect) )
     { int bute;
       if (module->nbr_oct_lu<TAILLE_ENTETE_PHIDGET)
            { bute = TAILLE_ENTETE_PHIDGET; }
       else { bute = TAILLE_ENTETE_PHIDGET + ntohs(module->response.taille); }

       if (bute>=sizeof(struct TRAME_PHIDGET_REPONSE))
        { Info_new( Config.log, Cfg_phidget.lib->Thread_debug, LOG_CRIT,
                   "%s: '%s': bute = %d >= %d (sizeof(module->reponse)=%d, taille recue = %d)", __func__, module->phidget.tech_id,
                    bute, sizeof(struct TRAME_PHIDGET_REPONSE), sizeof(module->response), ntohs(module->response.taille) );
          Deconnecter_module( module );
          return;
        }
       ...
       if (cpt>=0)
        { module->nbr_oct_lu += cpt;
          if (module->nbr_oct_lu >= TAILLE_ENTETE_PHIDGET + ntohs(module->response.taille))
           { Phidget_Processer_trame( module );                                      /* Si l'on a trouvé une trame complète !! */
             module->nbr_oct_lu = 0;
           }
        }
       else
        { Info_new( Config.log, Cfg_phidget.lib->Thread_debug, LOG_WARNING,
                    "%s: '%s': Read Error for %d. Get %d, error %s", __func__, module->phidget.tech_id,
                    module->phidget.id, cpt, strerror(errno) );
          Deconnecter_module ( module );
        }
      }
  }
/******************************************************************************************************************************/
/* Run_phidget_thread: Fait tourner un module phidget particulier                                                               */
/******************************************************************************************************************************/
 static void Run_phidget_thread ( struct MODULE_PHIDGET *module )
  { gchar thread_name[30];
    g_snprintf( thread_name, sizeof(thread_name), "W-PHIDGET%02d", module->phidget.id );
    ...
    Info_new( Config.log, Cfg_phidget.lib->Thread_debug, LOG_INFO, "%s: '%s' Started", __func__, module->phidget.tech_id );

    if (Dls_auto_create_plugin( module->phidget.tech_id, "Gestion du Wago" ) == FALSE)
     { Info_new( Config.log, Cfg_phidget.lib->Thread_debug, LOG_ERR, "%s: %s: DLS Create ERROR\n", module->phidget.tech_id ); }
    Mnemo_auto_create_WATCHDOG ( FALSE, module->phidget.tech_id, "IO_COMM", "Statut de la communication avec le Wago" );

    while(Cfg_phidget.lib->Thread_run == TRUE && Cfg_phidget.lib->Thread_reload == FALSE)      /* On tourne tant que necessaire */
     { sched_yield();
       usleep(module->delai);
       ...
       if (Partage->top>=module->last_top+10)                                                        /* Toutes les 1 secondes */
        { module->nbr_request_par_sec = module->nbr_request;
          module->nbr_request = 0;
          if(module->nbr_request_par_sec > module->phidget.max_request_par_sec) module->delai += 50;
          else if(module->delai>0) module->delai -= 50;
          module->last_top = Partage->top;
        }
       if ( module->phidget.enable == FALSE && module->started )                                     /* Module a deconnecter ! */
        { Deconnecter_module ( module );
          continue;
        }

       if ( module->phidget.enable == FALSE ||                          /* Si module DOWN ou si UP mais dans le delai de retry */
...
/********************************************* Début de l'interrogation du module *********************************************/
       if ( ! module->started )                                                                  /* Communication OK ou non ? */
        { if ( ! Connecter_module( module ) )
           { Info_new( Config.log, Cfg_phidget.lib->Thread_debug, LOG_INFO, "%s: '%s': Module %03d DOWN. retrying in %ds",
                       __func__, module->phidget.tech_id, module->phidget.id, PHIDGET_RETRY/10 );
             module->date_retente = Partage->top + PHIDGET_RETRY;
           }
        }
       else
        { if ( module->request )                                                         /* Requete en cours pour ce module ? */
           ...
           { if (module->date_next_eana<Partage->top)                                  /* Gestion décalée des I/O Analogiques */
             ...
             switch (module->mode)
              { case PHIDGET_GET_DESCRIPTION: Interroger_description( module ); break;
                case PHIDGET_GET_FIRMWARE   : Interroger_firmware( module ); break;
                case PHIDGET_INIT_WATCHDOG1 : Init_watchdog1( module ); break;
                case PHIDGET_INIT_WATCHDOG2 : Init_watchdog2( module ); break;
                case PHIDGET_INIT_WATCHDOG3 : Init_watchdog3( module ); break;
                case PHIDGET_INIT_WATCHDOG4 : Init_watchdog4( module ); break;
                case PHIDGET_GET_NBR_AI     : Interroger_nbr_entree_ANA( module ); break;
                case PHIDGET_GET_NBR_AO     : Interroger_nbr_sortie_ANA( module ); break;
                case PHIDGET_GET_NBR_DI     : Interroger_nbr_entree_TOR( module ); break;
                case PHIDGET_GET_NBR_DO     : Interroger_nbr_sortie_TOR( module ); break;
                case PHIDGET_GET_DI         : if (module->nbr_entree_tor) Interroger_entree_tor( module );
                                             else module->mode = PHIDGET_GET_AI;
                                             break;
                case PHIDGET_GET_AI         : if (module->nbr_entree_ana && module->do_check_eana)
                                                  Interroger_entree_ana( module );
                                             else module->mode = PHIDGET_SET_DO;
                                             break;
                case PHIDGET_SET_DO         : if (module->nbr_sortie_tor) Interroger_sortie_tor( module );
                                             else module->mode = PHIDGET_SET_AO;
                                             break;
                case PHIDGET_SET_AO         : if (module->nbr_sortie_ana && module->do_check_eana)
                                              { Interroger_sortie_ana( module );
                                              }
                                             else module->mode = PHIDGET_GET_DI;
...
/* Sortie: rien                                                                                                               */
/******************************************************************************************************************************/
 static void Decharger_tous_PHIDGET ( void  )
  { struct MODULE_PHIDGET *module;
    while ( Cfg_phidget.Modules_PHIDGET )
     { module = (struct MODULE_PHIDGET *)Cfg_phidget.Modules_PHIDGET->data;
       Info_new( Config.log, Cfg_phidget.lib->Thread_debug, LOG_DEBUG,
                 "%s: Wait for sub-process end : W-PHIDGET%02d '%s'", __func__, module->phidget.id, module->phidget.hostname );
       pthread_join( module->TID, NULL );                                                              /* Attente fin du fils */
       Info_new( Config.log, Cfg_phidget.lib->Thread_debug, LOG_NOTICE,
                 "%s: Sub-process ended : W-PHIDGET%02d '%s'", __func__, module->phidget.id, module->phidget.hostname );
       Cfg_phidget.Modules_PHIDGET = g_slist_remove ( Cfg_phidget.Modules_PHIDGET, module );
       g_free(module);
     }
  }
#endif
...
static void CCONV onVoltageChangeHandler(PhidgetDigitalInputHandle ph, void *ctx, int state) {
    printf("State: %d\n", state);
}
/******************************************************************************************************************************/
...
//    Cfg_phidget.Modules_PHIDGET = NULL;                                                       /* Init des variables du thread */
...
PhidgetNet_addServer("ServerName", "192.168.2.20", 5661, "passwd", 0);
 ...
 PhidgetRFIDHandle ch2;
 PhidgetRFID_create(&ch2);
