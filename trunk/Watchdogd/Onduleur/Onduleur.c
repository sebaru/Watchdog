...
/************************************************** PREPARE LES AI ************************************************************/
    if (!ups->nbr_connexion)
     { Info_new( Config.log, Cfg_ups.lib->Thread_debug, LOG_INFO,
       ...
       Zmq_Send_AI_to_master ( Cfg_ups.lib->zmq_to_master, NOM_THREAD, ups->tech_id, "LOAD", 0.0, FALSE );

       Mnemo_auto_create_AI ( FALSE, ups->tech_id, "REALPOWER", "Charge onduleur", "W" );
       Zmq_Send_AI_to_master ( Cfg_ups.lib->zmq_to_master, NOM_THREAD, ups->tech_id, "REALPOWER", 0.0, FALSE );

       Mnemo_auto_create_AI ( FALSE, ups->tech_id, "BATTERY_CHARGE", "Charge batterie", "%" );
       Zmq_Send_AI_to_master ( Cfg_ups.lib->zmq_to_master, NOM_THREAD, ups->tech_id, "BATTERY_CHARGE", 0.0, FALSE );

       Mnemo_auto_create_AI ( FALSE, ups->tech_id, "INPUT_VOLTAGE", "Tension d'entrée", "V" );
       Zmq_Send_AI_to_master ( Cfg_ups.lib->zmq_to_master, NOM_THREAD, ups->tech_id, "INPUT_VOLTAGE", 0.0, FALSE );

       Mnemo_auto_create_AI ( FALSE, ups->tech_id, "BATTERY_RUNTIME", "Durée de batterie restante", "s" );
       Zmq_Send_AI_to_master ( Cfg_ups.lib->zmq_to_master, NOM_THREAD, ups->tech_id, "BATTERY_RUNTIME", 0.0, FALSE );

       Mnemo_auto_create_AI ( FALSE, ups->tech_id, "BATTERY_VOLTAGE", "Tension batterie", "V" );
       Zmq_Send_AI_to_master ( Cfg_ups.lib->zmq_to_master, NOM_THREAD, ups->tech_id, "BATTERY_VOLTAGE", 0.0, FALSE );

       Mnemo_auto_create_AI ( FALSE, ups->tech_id, "INPUT_HZ", "Fréquence d'entrée", "HZ" );
       Zmq_Send_AI_to_master ( Cfg_ups.lib->zmq_to_master, NOM_THREAD, ups->tech_id, "INPUT_HZ", 0.0, FALSE );

       Mnemo_auto_create_AI ( FALSE, ups->tech_id, "OUTPUT_CURRENT", "Courant de sortie", "A" );
       Zmq_Send_AI_to_master ( Cfg_ups.lib->zmq_to_master, NOM_THREAD, ups->tech_id, "OUTPUT_CURRENT", 0.0, FALSE );

       Mnemo_auto_create_AI ( FALSE, ups->tech_id, "OUTPUT_HZ", "Fréquence de sortie", "HZ" );
       Zmq_Send_AI_to_master ( Cfg_ups.lib->zmq_to_master, NOM_THREAD, ups->tech_id, "OUTPUT_HZ", 0.0, FALSE );

       Mnemo_auto_create_AI ( FALSE, ups->tech_id, "OUTPUT_VOLTAGE", "Tension de sortie", "V" );
       Zmq_Send_AI_to_master ( Cfg_ups.lib->zmq_to_master, NOM_THREAD, ups->tech_id, "OUTPUT_VOLTAGE", 0.0, FALSE );
...
/******************************************************************************************************************************/
 static void Envoyer_sortie_aux_ups( void )
  { gchar buffer[512];
    ...
    JsonNode *request = Recv_zmq_with_json( Cfg_ups.lib->zmq_from_bus, NOM_THREAD, (gchar *)&buffer, sizeof(buffer) );
    if (request)
 ...
 static gboolean Interroger_ups( struct MODULE_UPS *ups )
  { gchar *reponse;
    ...
    if ( (reponse = Onduleur_get_var ( ups, "ups.load" )) != NULL )
     { Zmq_Send_AI_to_master ( Cfg_ups.lib->zmq_to_master, NOM_THREAD, ups->tech_id, "LOAD", atof(reponse+1), TRUE ); }

    if ( (reponse = Onduleur_get_var ( ups, "ups.realpower" )) != NULL )
     { Zmq_Send_AI_to_master ( Cfg_ups.lib->zmq_to_master, NOM_THREAD, ups->tech_id, "REALPOWER", atof(reponse+1), TRUE ); }

    if ( (reponse = Onduleur_get_var ( ups, "battery.charge" )) != NULL )
     { Zmq_Send_AI_to_master ( Cfg_ups.lib->zmq_to_master, NOM_THREAD, ups->tech_id, "BATTERY_CHARGE", atof(reponse+1), TRUE ); }

    if ( (reponse = Onduleur_get_var ( ups, "input.voltage" )) != NULL )
     { Zmq_Send_AI_to_master ( Cfg_ups.lib->zmq_to_master, NOM_THREAD, ups->tech_id, "INPUT_VOLTAGE", atof(reponse+1), TRUE ); }

    if ( (reponse = Onduleur_get_var ( ups, "battery.runtime" )) != NULL )
     { Zmq_Send_AI_to_master ( Cfg_ups.lib->zmq_to_master, NOM_THREAD, ups->tech_id, "BATTERY_RUNTIME", atof(reponse+1), TRUE ); }

    if ( (reponse = Onduleur_get_var ( ups, "battery.voltage" )) != NULL )
     { Zmq_Send_AI_to_master ( Cfg_ups.lib->zmq_to_master, NOM_THREAD, ups->tech_id, "BATTERY_VOLTAGE", atof(reponse+1), TRUE ); }

    if ( (reponse = Onduleur_get_var ( ups, "input.frequency" )) != NULL )
     { Zmq_Send_AI_to_master ( Cfg_ups.lib->zmq_to_master, NOM_THREAD, ups->tech_id, "INPUT_HZ", atof(reponse+1), TRUE ); }

    if ( (reponse = Onduleur_get_var ( ups, "output.current" )) != NULL )
     { Zmq_Send_AI_to_master ( Cfg_ups.lib->zmq_to_master, NOM_THREAD, ups->tech_id, "OUTPUT_CURRENT", atof(reponse+1), TRUE ); }

    if ( (reponse = Onduleur_get_var ( ups, "output.frequency" )) != NULL )
     { Zmq_Send_AI_to_master ( Cfg_ups.lib->zmq_to_master, NOM_THREAD, ups->tech_id, "OUTPUT_HZ", atof(reponse+1), TRUE ); }

    if ( (reponse = Onduleur_get_var ( ups, "output.voltage" )) != NULL )
     { Zmq_Send_AI_to_master ( Cfg_ups.lib->zmq_to_master, NOM_THREAD, ups->tech_id, "OUTPUT_VOLTAGE", atof(reponse+1), TRUE ); }

/*---------------------------------------------- Récupération des entrées TOR de l'UPS ---------------------------------------*/
    if ( (reponse = Onduleur_get_var ( ups, "outlet.1.status" )) != NULL )
     { Zmq_Send_DI_to_master ( Cfg_ups.lib->zmq_to_master, NOM_THREAD, ups->tech_id, "OUTLET_1_STATUS", !strcmp(reponse, "\"on\"") ); }

    if ( (reponse = Onduleur_get_var ( ups, "outlet.2.status" )) != NULL )
     { Zmq_Send_DI_to_master ( Cfg_ups.lib->zmq_to_master, NOM_THREAD, ups->tech_id, "OUTLET_2_STATUS", !strcmp(reponse, "\"on\"") ); }

    if ( (reponse = Onduleur_get_var ( ups, "ups.status" )) != NULL )
     { Zmq_Send_DI_to_master ( Cfg_ups.lib->zmq_to_master, NOM_THREAD, ups->tech_id, "UPS_ONLINE",       (g_strrstr(reponse, "OL")?TRUE:FALSE) );
       Zmq_Send_DI_to_master ( Cfg_ups.lib->zmq_to_master, NOM_THREAD, ups->tech_id, "UPS_CHARGING",     (g_strrstr(reponse, "CHRG")?TRUE:FALSE) );
       Zmq_Send_DI_to_master ( Cfg_ups.lib->zmq_to_master, NOM_THREAD, ups->tech_id, "UPS_ON_BATT",      (g_strrstr(reponse, "OB")?TRUE:FALSE) );
