 ...
 */
 ...
 static GSList *Alias=NULL;                                                  /* Liste des alias identifiÃ©s dans le source DLS */
 static GSList *Liste_Actions_bit    = NULL;                              /* Liste des actions rencontrÃ©es dans le source DLS */
 static GSList *Liste_Actions_num    = NULL;                              /* Liste des actions rencontrÃ©es dans le source DLS */
 static GSList *Liste_Actions_msg    = NULL;                              /* Liste des actions rencontrÃ©es dans le source DLS */
 static GSList *Liste_edge_up_bi     = NULL;                               /* Liste des bits B utilisÃ©s avec l'option EDGE_UP */
 static GSList *Liste_edge_up_entree = NULL;                               /* Liste des bits E utilisÃ©s avec l'option EDGE_UP */
...
/******************************************************************************************************************************/
/* New_chaine: Alloue une certaine quantitÃ© de mÃ©moire pour utiliser des chaines de caractÃ¨res                                */
/* EntrÃ©es: la longueur souhaitÃ©e                                                                                             */
...
/******************************************************************************************************************************/
/* Emettre: Met a jour le fichier temporaire en code intermÃ©diaire                                                            */
/* EntrÃ©es: la ligne d'instruction Ã  mettre                                                                                   */
/* Sortie: void                                                                                                               */
/******************************************************************************************************************************/
...
/******************************************************************************************************************************/
/* DlsScanner_error: AppellÃ© par le scanner en cas d'erreur de syntaxe (et non une erreur de grammaire !)                     */
/* EntrÃ©e : la chaine source de l'erreur de syntaxe                                                                           */
...
/* EntrÃ©e: le numÃ©ro de ligne, le format et les paramÃ¨tres associÃ©s                                                           */
/******************************************************************************************************************************/
...
/******************************************************************************************************************************/
/* New_option: Alloue une certaine quantitÃ© de mÃ©moire pour les options                                                       */
/* EntrÃ©es: rien                                                                                                              */
/* Sortie: NULL si probleme                                                                                                   */
/******************************************************************************************************************************/
...
/* EntrÃ©es: la liste des options, le type a rechercher                                                                        */
/* Sortie: -1 si pas trouvÃ©                                                                                                   */
/******************************************************************************************************************************/
...
/******************************************************************************************************************************/
/* New_condition_bi: Prepare la chaine de caractere associÃ©e Ã  la condition, en respectant les options                        */
/* EntrÃ©es: numero du bit bistable et sa liste d'options                                                                      */
...
/******************************************************************************************************************************/
 gchar *New_condition_bi( int barre, struct ALIAS *alias, GList *options )
  { gchar *result;
    gint taille;
    if (alias->num != -1) /* Alias par numÃ©ro ? */
...
/******************************************************************************************************************************/
/* New_condition_mono: Prepare la chaine de caractere associÃ©e Ã  la condition, en respectant les options                      */
/* EntrÃ©es: l'alias du monostable et sa liste d'options                                                                       */
/* Sortie: la chaine de caractere en C                                                                                        */
/******************************************************************************************************************************/
...
/******************************************************************************************************************************/
/* New_condition_tempo: Prepare la chaine de caractere associÃ©e Ã  la condition, en respectant les options                     */
/* EntrÃ©es: l'alias de la temporisatio et sa liste d'options                                                                  */
/* Sortie: la chaine de caractere en C                                                                                        */
/******************************************************************************************************************************/
...
/******************************************************************************************************************************/
/* New_condition_horloge: Prepare la chaine de caractere associÃ©e Ã  la condition, en respectant les options                   */
/* EntrÃ©es: l'alias de l'horloge et sa liste d'options                                                                        */
/* Sortie: la chaine de caractere en C                                                                                        */
/******************************************************************************************************************************/
...
/* EntrÃ©es: numero du monostable, sa logique                                                                                  */
...
/******************************************************************************************************************************/
/* New_action: Alloue une certaine quantitÃ© de mÃ©moire pour les actions DLS                                                   */
/* EntrÃ©es: rien                                                                                                              */
/* Sortie: NULL si probleme                                                                                                   */
/******************************************************************************************************************************/
...
/******************************************************************************************************************************/
 struct ACTION *New_action_msg_by_alias( struct ALIAS *alias )
  { struct ACTION *action;
    ...
    if (alias->type == ALIAS_TYPE_STATIC)                                                               /* Alias par numÃ©ro ? */
...
/******************************************************************************************************************************/
/* Add_bit_to_list: Ajoute un bit dans la liste des bits utilisÃ©                                                              */
/* EntrÃ©es: le type de bit et son numÃ©ro                                                                                      */
...
/* EntrÃ©es: numero de la sortie, sa logique                                                                                   */
/* Sortie: la structure action                                                                                                */
/******************************************************************************************************************************/
...
/******************************************************************************************************************************/
/* New_action_sortie: Prepare la structure ACTION associÃ©e Ã  l'alias en paremetre                                             */
/* EntrÃ©es: l'alias, le complement si besoin, les options                                                                     */
/* Sortie: la structure ACTION associÃ©e                                                                                       */
/******************************************************************************************************************************/
...
/* EntrÃ©es: numero du compteur d'impulsion, sa logique, son reset                                                             */
/* Sortie: la structure action                                                                                                */
/******************************************************************************************************************************/
...
/* EntrÃ©es: numero de la tempo, sa consigne                                                                                   */
/* Sortie: la structure action                                                                                                */
/******************************************************************************************************************************/
 struct ACTION *New_action_bus( struct ALIAS *alias, GList *options )
  { struct ACTION *result;
    ...
    tag    = Get_option_chaine ( options, T_TAG    );
...
/******************************************************************************************************************************/
/* New_alias: Alloue une certaine quantitÃ© de mÃ©moire pour utiliser des alias                                                 */
/* EntrÃ©es: le nom de l'alias, le tableau et le numero du bit                                                                 */
...
/******************************************************************************************************************************/
/* Get_alias: Recherche un alias donnÃ© en paramÃ¨tre                                                                           */
/* EntrÃ©es: le nom de l'alias                                                                                                 */
/* Sortie: NULL si probleme                                                                                                   */
/******************************************************************************************************************************/
...
/******************************************************************************************************************************/
/* Liberer_alias: Liberation de toutes les zones de mÃ©moire prÃ©cÃ©demment allouÃ©es                                             */
/* EntrÃ©es: kedal                                                                                                             */
/* Sortie: rien                                                                                                               */
/******************************************************************************************************************************/
...
/******************************************************************************************************************************/
/* Liberer_memoire: Liberation de toutes les zones de mÃ©moire prÃ©cÃ©demment allouÃ©es                                           */
/* EntrÃ©es: kedal                                                                                                             */
/* Sortie: rien                                                                                                               */
/******************************************************************************************************************************/
...
/******************************************************************************************************************************/
 gint Traduire_DLS( int id )
  { gchar source[80], cible[80], log[80];
    ...
    pthread_mutex_lock( &Partage->com_dls.synchro_traduction );                           /* Attente unicitÃ© de la traduction */

    Alias = NULL;                                                                                  /* Par dÃ©faut, pas d'alias */
    Liste_Actions_bit = NULL;                                                                    /* Par dÃ©faut, pas d'actions */
    Liste_Actions_num = NULL;                                                                    /* Par dÃ©faut, pas d'actions */
    Liste_Actions_msg = NULL;                                                                    /* Par dÃ©faut, pas d'actions */
    Liste_edge_up_bi  = NULL;                                               /* Liste des bits B utilisÃ© avec l'option EDGE UP */
    DlsScanner_set_lineno(1);                                                                     /* Reset du numÃ©ro de ligne */
    nbr_erreur = 0;                                                                   /* Au dÃ©part, nous n'avons pas d'erreur */
    rc = fopen( source, "r" );
    ...
    if (nbr_erreur)
     ...
     { gint fd;
       Emettre_erreur_new( "No error found" );                        /* Pas d'erreur rencontrÃ© (mais peu etre des warning !) */
       retour = TRAD_DLS_OK;
       ...
       if (fd<0)
        ...
        { gchar *include = " #include <Module_dls.h>\n";
          ...
          liste = Liste_Actions_bit;                                       /* Initialise les tableaux des actions rencontrÃ©es */
          while(liste)
           { gchar chaine[12];
          ...
          liste = Liste_Actions_num;                                       /* Initialise les tableaux des actions rencontrÃ©es */
          while(liste)
           { gchar chaine[12];
             g_snprintf(chaine, sizeof(chaine), "%d, ", GPOINTER_TO_INT(liste->data) );
             write(fd, chaine, strlen(chaine) );                                                      /* Ecriture du prologue */
          ...
          liste = Liste_Actions_msg;                                       /* Initialise les tableaux des actions rencontrÃ©es */
          while(liste)
           { gchar chaine[12];
             g_snprintf(chaine, sizeof(chaine), "%d, ", GPOINTER_TO_INT(liste->data) );
             write(fd, chaine, strlen(chaine) );                                                      /* Ecriture du prologue */
       ...
       liste = Alias;                                           /* LibÃ©ration des alias, et remontÃ© d'un Warning si il y en a */
       while(liste)
          ...
          if (alias->type == ALIAS_TYPE_DYNAMIC && !strcmp(alias->tech_id, Dls_plugin.tech_id))/* Alias Dynamiques uniquement */
           { switch(alias->bit)
              { case MNEMO_BUS: break;
                case MNEMO_SORTIE:
                 { Mnemo_auto_create_DO ( Dls_plugin.id, alias->acronyme, Get_option_chaine( alias->options, T_LIBELLE ) );
                   break;
                 }
                case MNEMO_ENTREE:
    ...
    g_free(Buffer);
    Buffer = NULL;
    pthread_mutex_unlock( &Partage->com_dls.synchro_traduction );                                         /* LibÃ©ration Mutex */
    return(retour);
  }
/*----------------------------------------------------------------------------------------------------------------------------*/
