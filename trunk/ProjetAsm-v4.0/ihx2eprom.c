...
/*    if (argc!=8)
    ...
    if (argc!=4)
     { printf("Usage: Ihx2eprom id_esclave type_AP [ram|rom]\n");
       exit(0);
     }
    ...
    type_AP        = atoi(argv[2]);
    switch( type_AP )
     { case 8: nbr_entre_ana  = 0;
               nbr_entre_tor  = 20;
               nbr_entre_choc = 0;
               nbr_sortie_tor = 8;
               nbr_sortie_ana = 0;
               break;
       default: printf("Mauvais numero d'AP"); exit(0);
     }

    printf("Creation du fichier de config avec:\n");
    printf("ID esclave %d type AP %d carte %d/%d/%d/%d/%d\n", id_esclave, type_AP,
    ...
    snprintf( chaine, sizeof(chaine), "TYPE_AP = %d\n", type_AP );
    write( id_fichier, chaine, strlen(chaine) );
