/******************************************************************************************************************************/
/* Client/Watchdog-client.c        Le client Watchdog v3.0                                                                    */
/* Projet WatchDog version 3.0       Gestion d'habitat                                                    01.04.2018 19:48:44 */
...
#endif
/******************************************************************************************************************************/
/*!Activate_fenetre: Creation de la fenetre gtk3                                                                              */
/******************************************************************************************************************************/
 static void Activate_fenetre (GtkApplication* app, gpointer user_data)
  { GtkWidget *window;
    gchar chaine[80];
    g_snprintf(chaine, sizeof(chaine), "Watchdog ver%s", VERSION);
    window = gtk_application_window_new (app);
    gtk_window_set_title (GTK_WINDOW (window), chaine );
...
#endif
    Info_new( Config_cli.log, Config_cli.log_override, LOG_INFO, "Main : Start v%s", VERSION );
    if (Config_cli.gui_tech == FALSE)
     { g_snprintf( Client.ident.nom, sizeof(Client.ident.nom), "%s", Config_cli.user );
       g_snprintf( Client.ident.passwd, sizeof(Client.ident.passwd), "%s", Config_cli.passwd );
       g_snprintf( Client.host, sizeof(Client.host), "%s", Config_cli.host );
...
/*    gtk_init( &argc, &argv );
    F_client = gtk_window_new (GTK_WINDOW_TOPLEVEL);
    g_signal_connect( G_OBJECT( F_client ), "delete_event", G_CALLBACK( gtk_main_quit ), NULL );
    g_signal_connect( G_OBJECT( F_client ), "destroy", G_CALLBACK( gtk_main_quit ), NULL );
    ...
    g_signal_connect (app, "activate", G_CALLBACK (Activate_fenetre), NULL);
...
#endif
    Info_new( Config_cli.log, Config_cli.log_override, LOG_NOTICE, "Main : Stopped" );
    return status;
  }
/*----------------------------------------------------------------------------------------------------------------------------*/
