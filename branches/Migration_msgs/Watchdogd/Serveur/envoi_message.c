...
/* Sortie: Niet                                                                                           */
/**********************************************************************************************************/
 void Proto_editer_message ( struct CLIENT *client, struct CMD_TYPE_MNEMO_MESSAGE *rezo_msg )
  { struct CMD_TYPE_MNEMO_MESSAGE *msg;
    ...
    if (msg)
     { Envoi_client( client, TAG_MESSAGE, SSTAG_SERVEUR_EDIT_MESSAGE_OK,
                  (gchar *)msg, sizeof(struct CMD_TYPE_MNEMO_MESSAGE) );
...
/* Sortie: Niet                                                                                           */
/**********************************************************************************************************/
 void Proto_valider_editer_message ( struct CLIENT *client, struct CMD_TYPE_MNEMO_MESSAGE *rezo_msg )
  { struct CMD_TYPE_MNEMO_MESSAGE *msg;
    gboolean retour;
#ifdef bouh
...
/* Sortie: Niet                                                                                           */
/**********************************************************************************************************/
 void Proto_effacer_message ( struct CLIENT *client, struct CMD_TYPE_MNEMO_MESSAGE *rezo_msg )
  { gboolean retour;
    ...
    if (retour)
     { Envoi_client( client, TAG_MESSAGE, SSTAG_SERVEUR_DEL_MESSAGE_OK,
                     (gchar *)rezo_msg, sizeof(struct CMD_TYPE_MNEMO_MESSAGE) );
     }
    else
     { struct CMD_GTK_MESSAGE erreur;
       g_snprintf( erreur.message, sizeof(erreur.message),
...
/* Sortie: Niet                                                                                           */
/**********************************************************************************************************/
 void Proto_ajouter_message ( struct CLIENT *client, struct CMD_TYPE_MNEMO_MESSAGE *rezo_msg )
  { struct CMD_TYPE_MNEMO_MESSAGE *msg;
    gint id;
#ifdef bouh
...
/**********************************************************************************************************/
 void *Envoyer_messages_thread ( struct CLIENT *client )
  { struct CMD_TYPE_MNEMO_MESSAGES *msgs;
    struct CMD_TYPE_MNEMO_MESSAGE *msg;
...
return(NULL);
    ...
    max_enreg = (Cfg_ssrv.taille_bloc_reseau - sizeof(struct CMD_TYPE_MNEMO_MESSAGES)) / sizeof(struct CMD_TYPE_MNEMO_MESSAGE);
    msgs = (struct CMD_TYPE_MNEMO_MESSAGES *)g_try_malloc0( Cfg_ssrv.taille_bloc_reseau );    
    if (!msgs)
     { struct CMD_GTK_MESSAGE erreur;
    ...
    do
     { msg = Recuperer_messageDB_suite( &db );                    /* Récupération d'un message dans la DB */
       if (msg)                                                /* Si enregegistrement, alors on le pousse */
        { memcpy ( &msgs->msg[msgs->nbr_messages], msg, sizeof(struct CMD_TYPE_MNEMO_MESSAGE) );
       ...
       if ( (msg == NULL) || msgs->nbr_messages == max_enreg )/* Si depassement de tampon ou plus d'enreg */
        { Envoi_client ( client, TAG_MESSAGE, SSTAG_SERVEUR_ADDPROGRESS_MESSAGE, (gchar *)msgs,
                         sizeof(struct CMD_TYPE_MNEMO_MESSAGES) + msgs->nbr_messages * sizeof(struct CMD_TYPE_MNEMO_MESSAGE) );
          msgs->nbr_messages = 0;
        }
     }
