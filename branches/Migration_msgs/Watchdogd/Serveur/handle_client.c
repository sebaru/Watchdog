...
/******************************************************************************************************************************/
 static void Envoyer_histo_au_client ( struct CLIENT *client, struct CMD_TYPE_HISTO *histo )
  { Info_new( Config.log, Cfg_ssrv.lib->Thread_debug, LOG_DEBUG,
             "%s: Histo traite : id = %08d, alive = %d, Acro=%s:%s, libelle=%s", __func__,
              histo->id, histo->alive, histo->msg.mnemo_base.acronyme, histo->msg.mnemo_base.dls_tech_id,
              histo->msg.mnemo_msg.libelle );
