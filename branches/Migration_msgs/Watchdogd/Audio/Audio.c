...
/******************************************************************************************************************************/
 gboolean Jouer_mp3 ( struct CMD_TYPE_MNEMO_MESSAGE *msg )
  { gchar nom_fichier[128];
    gint fd_cible, pid;

    g_snprintf( nom_fichier, sizeof(nom_fichier), "Son/%s.mp3", msg->acronyme );
    ...
    while(Cfg_audio.lib->Thread_run == TRUE)                                                 /* On tourne tant que necessaire */
     { gchar buffer[256];
       ...
       Info_new( Config.log, Cfg_audio.lib->Thread_debug, LOG_DEBUG,
                ...
                histo->msg.mnemo_base.dls_tech_id, histo->msg.mnemo_base.acronyme, histo->msg.mnemo_msg.audio, histo->alive );
                   
       if ( M(NUM_BIT_M_AUDIO_INHIB) == 1 &&
           ! (histo->msg.mnemo_msg.type == MSG_ALERTE || histo->msg.mnemo_msg.type == MSG_DANGER ||
              histo->msg.mnemo_msg.type == MSG_ALARME)
          )                                                                     /* Bit positionné quand arret diffusion audio */
        { Info_new( Config.log, Cfg_audio.lib->Thread_debug, LOG_WARNING,
                   "%s : Envoi audio inhibe pour '%s:%s' (histo->msg.audio=%d)", __func__,
                   histo->msg.mnemo_base.dls_tech_id, histo->msg.mnemo_base.acronyme, histo->msg.mnemo_msg.audio );
          continue;
        }
       if ( histo->alive == 1 && histo->msg.mnemo_msg.audio )                                       /* Si le message apparait */
        { Info_new( Config.log, Cfg_audio.lib->Thread_debug, LOG_INFO,
                   "%s : Envoi du message '%s:%s' (histo->msg.audio=%d)", __func__,
                   histo->msg.mnemo_base.dls_tech_id, histo->msg.mnemo_base.acronyme, histo->msg.mnemo_msg.audio );

          if (Config.instance_is_master)
           { Envoyer_commande_dls( histo->msg.mnemo_msg.bit_audio );         /* Positionnement du profil audio via monostable */
          ...
          if (Jouer_mp3 ( &histo->msg.mnemo_msg ) == FALSE)            /* Par priorité : mp3 d'abord, synthèse vocale ensuite */
           { if (strlen(histo->msg.mnemo_msg.libelle_audio))/* Si libelle_audio, le jouer, sinon jouer le libelle tout court) */
              { Jouer_google_speech( histo->msg.mnemo_msg.libelle_audio ); }
             else
              { Jouer_google_speech( histo->msg.mnemo_base.libelle ); }
           }
        }
       else
        { Info_new( Config.log, Cfg_audio.lib->Thread_debug, LOG_DEBUG,
                    "%s : Msg Audio non envoye '%s:%s' (histo->msg.audio=%d)", __func__,
                    histo->msg.mnemo_base.dls_tech_id, histo->msg.mnemo_base.acronyme, histo->msg.mnemo_msg.audio );
        }
     }
    Close_zmq ( zmq_msg );
