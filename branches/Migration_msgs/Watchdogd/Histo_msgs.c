  ...
  { gchar *nom_ack;
    ...
    if (ajout == TRUE)
     { nom_ack = Normaliser_chaine ( histo->nom_ack );                                       /* Formatage correct des chaines */
       ...
       g_snprintf( requete, sizeof(requete),                                                                   /* Requete SQL */
                   "INSERT INTO %s(alive,mnemo_id,nom_ack,date_create_sec,date_create_usec)"
                   " VALUES "
                   "('%d','%d','%s','%d','%d')", NOM_TABLE_HISTO_MSGS, TRUE,
                   histo->msg.mnemo_base.id, 
     ...
     { 
       if (histo->alive == TRUE)
        ...
        { g_snprintf( requete, sizeof(requete),                                                                /* Requete SQL */
                      ...
                      " WHERE histo.alive=1 AND histo.mnemo_id='%d'",
                       NOM_TABLE_HISTO_MSGS,
                      (int)histo->date_fin, histo->msg.mnemo_base.id );
        }
     }
...
/******************************************************************************************************************************/
 struct CMD_TYPE_HISTO *Rechercher_histo_msgsDB_by_id ( guint id )
  { struct CMD_TYPE_HISTO *histo;
    ...
    g_snprintf( requete, sizeof(requete),                                                  /* Requete SQL */
                "SELECT histo.id, histo.alive, mnemo.acronyme, msg.libelle, msg.type, dls.syn_id,"
                "syn.groupe, syn.page, histo.nom_ack, histo.date_create_sec, histo.date_create_usec,"
                "histo.date_fixe,histo.date_fin,dls.shortname"
                " FROM %s as histo"
                " INNER JOIN %s as msg ON msg.mnemo_id = mnemo.id"
                " INNER JOIN %s as mnemo ON histo.mnemo_id = mnemo.id"
                " INNER JOIN %s as dls ON dls.id = mnemo.dls_id"
                " INNER JOIN %s as syn ON syn.id = dls.syn_id"
                " WHERE histo.id = %d",
 ...
 struct CMD_TYPE_HISTO *Recuperer_histo_msgsDB_suite( struct DB **db_orig )
  { struct CMD_TYPE_HISTO *histo_msgs;
    ...
    if (!histo_msgs) Info_new( Config.log, Config.log_msrv, LOG_ERR,
     ...
     { g_snprintf( histo_msgs->msg.mnemo_base.libelle,         sizeof(histo_msgs->msg.mnemo_base.libelle),         "%s", db->row[3]  );
       g_snprintf( histo_msgs->msg.mnemo_base.syn_parent_page, sizeof(histo_msgs->msg.mnemo_base.syn_parent_page), "%s", db->row[6]  );
       g_snprintf( histo_msgs->msg.mnemo_base.syn_page,        sizeof(histo_msgs->msg.mnemo_base.syn_page),        "%s", db->row[7]  );
       g_snprintf( histo_msgs->nom_ack,                        sizeof(histo_msgs->nom_ack),                        "%s", db->row[8]  );
       g_snprintf( histo_msgs->msg.mnemo_base.dls_shortname,   sizeof(histo_msgs->msg.mnemo_base.dls_shortname),   "%s", db->row[13] );
       g_snprintf( histo_msgs->msg.mnemo_base.acronyme,        sizeof(histo_msgs->msg.mnemo_base.acronyme),        "%s", db->row[2] );
       histo_msgs->id               = atoi(db->row[0]);
       histo_msgs->alive            = atoi(db->row[1]);
       histo_msgs->msg.mnemo_msg.type         = atoi(db->row[4]);
       histo_msgs->msg.mnemo_base.syn_id       = atoi(db->row[5]);
