...
/* Sortie: Niet                                                                                                               */
/******************************************************************************************************************************/
 static gboolean Envoi_sms_gsm ( struct CMD_TYPE_MNEMO_MESSAGE *msg, gchar *telephone )
  { GSM_SMSMessage sms;
    GSM_Error error;
    GSM_SMSC PhoneSMSC;
    GSM_StateMachine *s;
...
/* Sortie: Niet                                                                                                               */
/******************************************************************************************************************************/
 static void Envoi_sms_smsbox ( struct CMD_TYPE_MNEMO_MESSAGE *msg, gchar *telephone )
...
/* Sortie : néant                                                                                                             */
/******************************************************************************************************************************/
 void Smsg_send_to_all_authorized_recipients ( struct CMD_TYPE_MNEMO_MESSAGE *msg )
  { struct SMSDB *sms;
    struct DB *db;
...
/********************************************************* Envoi de SMS *******************************************************/
       ...
       if ( histo && histo->alive == TRUE && histo->msg.mnemo_msg.sms != MSG_SMS_NONE)      /* On n'envoie que si MSGnum == 1 */
        { Info_new( Config.log, Cfg_smsg.lib->Thread_debug, LOG_INFO,
                   "%s : Sending msg '%s:%s' (%s)", __func__, histo->msg.mnemo_base.dls_tech_id, histo->msg.mnemo_base.acronyme,
                   histo->msg.mnemo_msg.libelle_sms );
      
/*************************************************** Envoi en mode GSM ********************************************************/
          if (Partage->top < TOP_MIN_ENVOI_SMS)
           { Info_new( Config.log, Cfg_smsg.lib->Thread_debug, LOG_INFO,
                      "%s: Envoi trop tot !! (%s)", __func__, histo->msg.mnemo_msg.libelle_sms ); }
          else 
           { Smsg_send_to_all_authorized_recipients( &histo->msg.mnemo_msg ); }
        }
       else
        { Info_new( Config.log, Cfg_smsg.lib->Thread_debug, LOG_DEBUG,
                    "%s : msg '%s:%s' not sent (alive=%d, msg.sms = %d) (%s)", __func__,
                    histo->msg.mnemo_base.dls_tech_id, histo->msg.mnemo_base.acronyme,
                    histo->alive, histo->msg.mnemo_msg.sms, histo->msg.mnemo_msg.libelle_sms );
        }
     }
    Close_zmq ( zmq_msg );
    Close_zmq ( zmq_admin );
