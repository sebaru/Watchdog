...
/******************************************************************************************************************************/
/* %s: Demarre le thread en paremetre                                                                                         */
  ...
  { pthread_attr_t attr;
    ...
    if (lib->Thread_run == TRUE)
     { Info_new( Config.log, Config.log_msrv, LOG_INFO,
                 "%s: thread %s already seems to be running", __func__, lib->nom_fichier );
       return(FALSE);
     }

    if ( pthread_attr_init(&attr) )                                                 /* Initialisation des attributs du thread */
     { Info_new( Config.log, Config.log_msrv, LOG_ERR,
                "%s: pthread_attr_init failed (%s)", __func__, lib->nom_fichier );
       return(FALSE);
     }

    if ( pthread_attr_setdetachstate(&attr, PTHREAD_CREATE_DETACHED) )                       /* On le laisse joinable au boot */
     { Info_new( Config.log, Config.log_msrv, LOG_ERR,
                "%s: pthread_setdetachstate failed (%s)", __func__, lib->nom_fichier );
       return(FALSE);
     }

    if ( pthread_create( &tid, &attr, (void *)lib->Run_thread, lib ) )
     { Info_new( Config.log, Config.log_msrv, LOG_ERR,
                "%s: pthread_create failed (%s)", __func__, lib->nom_fichier );
       return(FALSE);
     }
    pthread_attr_destroy(&attr);                                                                        /* Libération mémoire */
    Info_new( Config.log, Config.log_msrv, LOG_NOTICE,
             "%s: Starting thread %s OK (TID=%p)", __func__, lib->nom_fichier, tid );
    return(TRUE);
  }
/******************************************************************************************************************************/
  ...
  { pthread_mutexattr_t attr;                                                          /* Initialisation des mutex de synchro */
    ...
    if (!lib->dl_handle)
     { Info_new( Config.log, Config.log_msrv, LOG_WARNING,
                 "%s: Candidat %s failed (%s)", __func__, nom_absolu, dlerror() );
       g_free(lib);
       return(NULL);
     }
    ...
    if (!lib->Run_thread)
     { Info_new( Config.log, Config.log_msrv, LOG_WARNING,
                "%s: Candidat %s rejected (Run_thread not found)", __func__, nom_absolu ); 
    ...
    if (!lib->Admin_command)
     { Info_new( Config.log, Config.log_msrv, LOG_WARNING,
                "%s: Candidat %s rejected (Admin_command not found)", __func__, nom_absolu ); 
       dlclose( lib->dl_handle );
       g_free(lib);
       return(NULL);
     }

    lib->Send_Output_Event = dlsym( lib->dl_handle, "Send_Output_Event" );                        /* Recherche de la fonction */
    if (!lib->Send_Output_Event)
     { Info_new( Config.log, Config.log_msrv, LOG_INFO,
                "%s: Candidat %s Send_Output_Event not found", __func__, nom_absolu ); 
     }
    ...
    while(liste)                                                                            /* Liberation mémoire des modules */
     { lib = (struct LIBRAIRIE *)liste->data;                                         /* Récupération des données de la liste */
       ...
       if ( ! strcmp ( lib->admin_prompt, prompt ) )
        { Info_new( Config.log, Config.log_msrv, LOG_INFO,
                    "%s: trying to unload %s", __func__, lib->nom_fichier );
          ...
          Info_new( Config.log, Config.log_msrv, LOG_NOTICE,
                    "%s: library %s unloaded", __func__, lib->nom_fichier );
          g_free( lib );
          return(TRUE);
        }
       liste = liste->next;
...
/******************************************************************************************************************************/
/* Envoyer_event_to_librairie: Envoi un event (gchar) vers la librairie dont le nom est en parametre                          */
/* Entrée: Le nom de la librairie et de l'evenement                                                                           */
/* Sortie: FALSE si rien n'a été transmis                                                                                     */
/******************************************************************************************************************************/
 gboolean Envoyer_event_to_librairie ( gchar *prompt, gchar *event )
  { struct LIBRAIRIE *lib;
    GSList *liste;
    ...
    while(liste)                                                                            /* Liberation mémoire des modules */
     { lib = (struct LIBRAIRIE *)liste->data;                                         /* Récupération des données de la liste */
       ...
       if ( ! strcmp ( lib->admin_prompt, prompt ) && lib->Send_Output_Event )
        { Info_new( Config.log, Config.log_msrv, LOG_INFO,
                    "%s: sending event '%s' to '%s'", __func__, event, prompt );
          lib->Send_Output_Event( event );
          return(TRUE);
        }
       liste = liste->next;
     }
