...
/******************************************************************************************************************************/
 struct SMSDB *Sms_Recuperer_smsDB_suite( struct DB *db )
  { struct SMSDB *sms;
    ...
    if (!sms) Info_new( Config.log, Cfg_sms.lib->Thread_debug, LOG_ERR, "%s: Erreur allocation mémoire", __func__ );
    else
...
/******************************************************************************************************************************/
 static void Traiter_commande_sms ( gchar *from, gchar *texte )
  { struct CMD_TYPE_MNEMO_BASE *mnemo;
    ...
    Info_new( Config.log, Cfg_sms.lib->Thread_debug, LOG_NOTICE,
             "%s: Received %s from %s(%s). Processing...",
              __func__, texte, sms->user_name, sms->user_sms_phone );
    g_free(sms);
    g_snprintf(response, sizeof(response), "Processing: %s", texte );                       /* Envoi de l'acquit de reception */
    Envoyer_sms_gsm_text ( response );
    ...
    nbr_result =  Map_event_to_mnemo_new ( &db, Config.instance_id, NOM_THREAD, texte );         /* Evenement pour ce texte ? */
    if (nbr_result == 0)
     { Info_new( Config.log, Cfg_sms.lib->Thread_debug, LOG_WARNING,
	               "%s: event '%s' not found", __func__, texte );
       g_snprintf(response, sizeof(response), "Event not found: '%s'", texte);
	      Envoyer_sms_gsm_text ( response );
     }
    else if (nbr_result == 1)
        ...
        { while ( (mnemo = Recuperer_mnemo_baseDB_suite( &db )) != NULL)
           { Info_new( Config.log, Cfg_sms.lib->Thread_debug, LOG_NOTICE,
             ...
             switch (mnemo->type)
              { case MNEMO_MONOSTABLE: Envoyer_commande_dls(mnemo->num); break;
		            		case MNEMO_ENTREE_ANA:
                 { g_snprintf( response, sizeof(response), "%s: %6.2f %s",
                               mnemo->libelle, Partage->ea[mnemo->num].val_ech, Partage->ea[mnemo->num].confDB.unite );
            	      Envoyer_sms_gsm_text ( response );
                   break;
                 }
		            		default: Info_new( Config.log, Cfg_sms.lib->Thread_debug, LOG_WARNING,
                                   "%s: Event '%s' not handled -> Type %d Num %d - %s",
                                    __func__, texte, mnemo->type, mnemo->num, mnemo->libelle );
              }
             g_free(mnemo);
           }
        }
        ...
        { while ( (mnemo = Recuperer_mnemo_baseDB_suite( &db )) != NULL)
           { Info_new( Config.log, Cfg_sms.lib->Thread_debug, LOG_NOTICE,
                      "%s: Match found for event '%s' -> Type %d Num %d - %s",
                       texte, mnemo->type, mnemo->num, mnemo->libelle );
             /* Accès URL via thread HTTP au master */ 
             g_free(mnemo);
           }
        }
      }
     ...
     { gchar chaine[128];
   	   g_snprintf( response, sizeof(response), "Precisez:\n");
       while ( (mnemo = Recuperer_mnemo_baseDB_suite( &db )) != NULL)
        { g_strlcat( response, mnemo->command_text+strlen(Config.instance_id)+strlen(NOM_THREAD)+2, sizeof(response) );
          g_free(mnemo);
        }
	      Envoyer_sms_gsm_text ( response );
     }
...
/******************************************************************************************************************************/
/* Envoyer_msg_via_gsm: Envoi un sms par le gsm                                                                               */
/* Entrée: le message à envoyer et le destinataire                                                                            */
/* Sortie: False si problème                                                                                                  */
/******************************************************************************************************************************/
 static gboolean Envoyer_msg_via_gsm ( struct CMD_TYPE_MESSAGE *msg, gchar *telephone )
  { struct gn_statemachine *state;
    gn_error error;
    gn_data data;
    gn_sms sms;
    ...
    if ((error=gn_lib_phoneprofile_load("", &state)) != GN_ERR_NONE)                                      /* Read config file */
     { Info_new( Config.log, Cfg_sms.lib->Thread_debug, LOG_WARNING,
                "Envoyer_msg_via_gsm: Read Phone profile NOK (%s)", gn_error_print(error) );
       return(FALSE);
     }

    if ((error=gn_lib_phone_open(state)) != GN_ERR_NONE)
     { Info_new( Config.log, Cfg_sms.lib->Thread_debug, LOG_WARNING,
                "Envoyer_msg_via_gsm: Open Phone NOK (%s)", gn_error_print(error) );
       return(FALSE);
     }
...
/*	sms.delivery_report = true; */
    ...
    if (error == GN_ERR_NONE)
     { Info_new( Config.log, Cfg_sms.lib->Thread_debug, LOG_INFO, "%s: Envoi SMS Ok to %s (%s)",
                 __func__, telephone, msg->libelle_sms ); }
    else
     { Info_new( Config.log, Cfg_sms.lib->Thread_debug, LOG_WARNING, "%s Envoi SMS Nok to %s (%s)",
                 __func__,  telephone, gn_error_print(error)); }
...
/********************************************** Chargement des informations en bases ******************************************/
    ...
    while ( (sms = Sms_Recuperer_smsDB_suite( db )) != NULL)
     { switch (msg->sms)
        { case MSG_SMS_YES:
               if ( Envoyer_msg_via_gsm ( msg, sms->user_sms_phone ) == FALSE )                            /* Priorité au gsm */
                { Envoi_sms_smsbox( msg, sms->user_sms_phone ); }
               break;
          case MSG_SMS_GSM_ONLY:
               Envoyer_msg_via_gsm   ( msg, sms->user_sms_phone );
               break;
...
/******************************************************************************************************************************/
/* Run_thread: Fonction principale du thread                                                                                  */
/* Entrée: la structure librairie du thread, partagée avec le master                                                          */
/* Sortie: Niet                                                                                                               */
/******************************************************************************************************************************/
     ...
     { usleep(10000);
       ...
       if (Cfg_sms.lib->Thread_sigusr1)                                                       /* A-t'on recu un signal USR1 ? */
        { int nbr;
          ...
          pthread_mutex_unlock( &Cfg_sms.lib->synchro );
          Info_new( Config.log, Cfg_sms.lib->Thread_debug, LOG_INFO, "%s: Nbr SMS restant a envoyer = %d", __func__, nbr );
...
/****************************************************** Envoi de SMS **********************************************************/
       ...
       Info_new( Config.log, Cfg_sms.lib->Thread_debug, LOG_INFO,
                "%s: Reste %d a envoyer apres le msg %d",
                 __func__, g_slist_length(Cfg_sms.Liste_histos), histo->msg.num );
       pthread_mutex_unlock( &Cfg_sms.lib->synchro );
