 ...
 void Clear_histoDB ( void )
  { struct DB *db;
    ...
    g_snprintf( requete, sizeof(requete),                                                  /* Requete SQL */
                "DELETE FROM %s WHERE alive=1", NOM_TABLE_HISTO_MSGS );
...
/**********************************************************************************************************/
 gboolean Retirer_histoDB ( guint id )
  { gchar requete[1024];
    ...
    g_snprintf( requete, sizeof(requete),                                                  /* Requete SQL */
                "DELETE FROM %s WHERE id=%d", NOM_TABLE_HISTO_MSGS, id );
 ...
 gboolean Ajouter_histoDB ( struct HISTODB *histo )
  { gchar requete[1024];
    ...
    g_snprintf( requete, sizeof(requete),                                                  /* Requete SQL */
                ...
                "date_fixe) VALUES "
                "(%d,'%s',%d,%d,'%s',%d,%d,0)", NOM_TABLE_HISTO_MSGS, histo->msg.num, libelle, 
                histo->msg.type,
    ...
    db = Init_DB_SQL();       
    if (!db)
     { Info_new( Config.log, Config.log_msrv, LOG_ERR, "Ajouter_histoDB: Connexion DB impossible" );
       return(FALSE);
     }
 ...
 gboolean Modifier_histoDB ( struct CMD_TYPE_HISTO *histo )
  { gchar requete[1024];
    ...
    g_snprintf( requete, sizeof(requete),                                                  /* Requete SQL */
                ...
                NOM_TABLE_HISTO_MSGS, nom_ack, (gint)histo->date_fixe, histo->id );
    g_free(nom_ack);
 ...
 gboolean Recuperer_histoDB ( struct DB **db_retour )
  { gchar requete[1024];
    ...
    g_snprintf( requete, sizeof(requete),                                                  /* Requete SQL */
                ...
                NOM_TABLE_HISTO_MSGS, NOM_TABLE_HISTO_MSGS, NOM_TABLE_SYNOPTIQUE, NOM_TABLE_SYNOPTIQUE,
