ASxxxx Assembler V01.70 + NoICE + SDCC mods + Flat24 Feb-1999  (Intel 8051), page 1.
                              ...
                              3 ; Sébastien Lefevre                                                            dim 13 jun 2004 17:02:11 CEST
                              4 ;-----------------------------------------------------------------------------------------------------------
...
ASxxxx Assembler V01.70 + NoICE + SDCC mods + Flat24 Feb-1999  (Intel 8051), page 12.
   ...
   04CD 22                  388         ret
                            389 ;-----------------------------------------------------------------------------------------------------------
                            390 ; _polling_entre_tor_test: Compare la valeur reelle d'un port avec celle sauvegardée et met à jour le cas
                            391 ; échéant                                                                                                   
                            392 ;-----------------------------------------------------------------------------------------------------------
   04CE                     393 _polling_entre_tor_test:
   04CE E6                  394         mov A,@R0
   04CF F9                  395         mov R1,A                             ; Sauvegarde
   04D0 54 02               396 	anl A,#0x02                          ; Bit 1: Locké par le PC ou non ??
                            397 	;;; desactivé pb fenetre cuisine 10/07/05 
                            398         ;;; jnz _polling_entre_tor_test_fin      ; Locké ? -> Pas touche !
ASxxxx Assembler V01.70 + NoICE + SDCC mods + Flat24 Feb-1999  (Intel 8051), page 13.



                            399 
   04D2 74 00               400         mov A,#0x00                          ; C possede la valeur reelle du port à tester
   ...
   04DA 60 04               407 	jz _polling_entre_tor_test_fin       ; rien n'a changé, on se barre
                            408 	
   ...
   04E1 22                  415         ret
                            ...
                            417 
                            418 ;-----------------------------------------------------------------------------------------------------------
                            419 ; _fct_entre_tor: Envoi de l'etat des entrées TOR
                            420 ;-----------------------------------------------------------------------------------------------------------
   ...
   04F0 76 00               432 	mov @R0, #0x00                ; mise à zero
   ...
   0506 54 FD               451         anl A,#0xFD                   ; mise à zero bit 1
...
ASxxxx Assembler V01.70 + NoICE + SDCC mods + Flat24 Feb-1999  (Intel 8051), page 14.
   ...
   0517 0D                  463 	inc R5                        ; l'octet est partiellement utilisé, il faut quand mem le compter
   0518 08                  464 	inc R0                        ; le crc est donc à l'octet suivant
   0519                     465 _fct_entre_tor_taille:
   0519 EA                  466         mov A,R2                      ; l'adresse du champs taille etait sauvée dans R2
   ...
   0524 F4                  480 	cpl A
   0525 FE                  481 	mov R6,A                      ; R6 = complément de R7
                            482 
   0526 E6                  483 	mov A,@R0                     ; recupération octet donnes trame
   0527 5E                  484 	anl A,R6                      ; mise à zero du bit de poids R7
   0528 F6                  485 	mov @R0,A
   0529 80 03               486 	sjmp _fct_entre_tor_push_bit_suite
   052B                     487 _fct_entre_tor_push_1:
   052B E6                  488 	mov A,@R0                     ; recupération octet donnes trame
   052C 4F                  489 	orl A,R7                      ; mise à un bit de poids R7
   052D F6                  490 	mov @R0,A
   052E                     491 _fct_entre_tor_push_bit_suite:
   052E EF                  492 	mov A,R7
   052F C3                  493 	clr C                         ; rotate du poids: << 1
   0530 13                  494 	rrc A
   0531 FF                  495 	mov R7,A                      ; decalage pour prochaine mise à un 
   ...
   0535 76 00               498 	mov @R0, #0x00                ; mise à zero
   0537 0D                  499 	inc R5                        ; un octet de plus consommé.
